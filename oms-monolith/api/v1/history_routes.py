...
from shared.exceptions import ServiceTimeoutError, DatabaseConnectionError, PermissionError as OmsPermissionError
...
async def revert_schema(
    ...
    try:
    ...
    except (PermissionError, OmsPermissionError) as e:
        raise HTTPException(
        ...
        logger.error(f"Database connection error during schema revert: {e}")
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Database service temporarily unavailable"
        )
    except TimeoutError as e:
        logger.error(f"Timeout during schema revert: {e}")
        raise HTTPException(
            status_code=status.HTTP_504_GATEWAY_TIMEOUT,
            detail="Schema revert operation timed out"
        )
    except KeyError as e:
        logger.error(f"Missing required field in revert request: {e}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Missing required field: {str(e)}"
        )
    except RuntimeError as e:
        logger.error(f"Unexpected error during schema revert: {str(e)}")
        raise HTTPException(
...
async def publish_audit_event(
    ...
    try:
        ...
        logger.error(f"Service connection error publishing audit event: {e}")
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Event publisher service temporarily unavailable"
        )
    except TimeoutError as e:
        logger.error(f"Timeout publishing audit event: {e}")
        raise HTTPException(
            status_code=status.HTTP_504_GATEWAY_TIMEOUT,
            detail="Event publishing timed out"
        )
    except ValueError as e:
        logger.error(f"Invalid event data: {e}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid event parameters"
        )
    except RuntimeError as e:
        logger.error(f"Unexpected error publishing audit event: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
