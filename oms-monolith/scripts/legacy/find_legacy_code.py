...
class LegacyCodeFinder:
    ...
    def generate_report(self):
        """분석 결과 리포트 생성"""
        logger.info("\n" + "=" * 70)
        logger.info("📊 Legacy Code Analysis Report")
        logger.info("=" * 70)
        ...
        if self.findings["large_files"]:
            ...
            for file_info in sorted_files[:10]:
                logger.info(f"{file_info['file']:.<50} {file_info['lines']} lines ({file_info['size_kb']:.1f} KB)")
        ...
        if self.findings["dead_code"]:
            ...
            for dead in self.findings["dead_code"][:10]:
                logger.info(f"{dead['file']}: {dead['type']} - {', '.join(dead['items'][:3])}")
        
        # Summary
        logger.info("\n📈 Summary:")
        logger.info("-" * 50)
        logger.info(f"Large files (>500 lines): {len(self.findings['large_files'])}")
        logger.info(f"Duplicate functionality groups: {len(self.findings['duplicate_functionality'])}")
        logger.info(f"Legacy patterns found: {len(self.findings['old_patterns'])}")
        logger.info(f"TODO/FIXME comments: {len(self.findings['todo_fixme'])}")
        logger.info(f"Potential dead code: {len(self.findings['dead_code'])}")
        
        # Recommendations
        logger.info("\n💡 Recommendations:")
        logger.info("-" * 50)
        logger.info("1. Review and refactor large files (>500 lines)")
        logger.info("2. Consolidate duplicate database clients and validation logic")
        logger.info("3. Update old string formatting to f-strings")
        logger.info("4. Remove commented-out code")
        logger.info("5. Address TODO/FIXME comments")
        logger.info("6. Consider splitting utils/helpers into specific modules")
