...
class DatabaseConfig:
    ...
    # Cache
    cache_type: str = "redis"  # redis, memory, terminus
    cache_ttl: int = 3600
...
class UnifiedConfigService:
    ...
    def _load_from_env(self, config: UnifiedConfig) -> None:
        """환경 변수에서 설정 로드"""
        # Application
        config.app.environment = Environment(os.getenv("ENVIRONMENT", "development").lower())
        config.app.debug = os.getenv("DEBUG", "false").lower() == "true"
        config.app.api_host = os.getenv("API_HOST", config.app.api_host)
        config.app.api_port = int(os.getenv("API_PORT", str(config.app.api_port)))
        
        # Database
        config.database.terminus_endpoint = os.getenv("TERMINUS_DB_ENDPOINT", config.database.terminus_endpoint)
        config.database.terminus_username = os.getenv("TERMINUS_DB_USER", config.database.terminus_username)
        config.database.terminus_password = os.getenv("TERMINUS_DB_PASSWORD", config.database.terminus_password)
        config.database.terminus_organization = os.getenv("TERMINUS_ORGANIZATION", config.database.terminus_organization)
        config.database.terminus_db = os.getenv("TERMINUS_DB", config.database.terminus_db)
        config.database.terminus_key = os.getenv("TERMINUS_DB_KEY", config.database.terminus_key)
        
        # Redis
        sentinels = os.getenv("REDIS_SENTINELS", "")
        if sentinels:
            config.database.redis_sentinels = [s.strip() for s in sentinels.split(",")]
        config.database.redis_master_name = os.getenv("REDIS_MASTER_NAME", config.database.redis_master_name)
        config.database.redis_password = os.getenv("REDIS_PASSWORD", config.database.redis_password)
        config.database.redis_db = int(os.getenv("REDIS_DB", str(config.database.redis_db)))
        
        # Security
        config.security.jwt_secret_key = os.getenv("JWT_SECRET_KEY", config.security.jwt_secret_key)
        config.security.mtls_enabled = os.getenv("MTLS_ENABLED", "false").lower() == "true"
        
        # Messaging
        config.messaging.nats_enabled = os.getenv("NATS_ENABLED", "true").lower() == "true"
        ...
        config.monitoring.log_format = os.getenv("LOG_FORMAT", config.monitoring.log_format)
    ...
    def _validate_config(self, config: UnifiedConfig) -> None:
        ...
        if config.security.mtls_enabled:
            if not all([config.security.mtls_cert_path, 
                        config.security.mtls_key_path,
                        config.security.mtls_ca_path]):
