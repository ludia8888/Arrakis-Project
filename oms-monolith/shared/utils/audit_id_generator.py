...
if __name__ == "__main__":
    ...
    logger.info(f"Generated ID: {audit_id}")
    ...
    logger.info(f"All object type ops: {AuditIDPatterns.all_object_type_operations()}")
    logger.info(f"All create ops: {AuditIDPatterns.all_create_operations()}")
    logger.info(f"Today's ops: {AuditIDPatterns.today_operations()}")
    
    # Anomaly detection keys
    keys = AuditIDGenerator.get_anomaly_detection_keys(audit_id)
    logger.info(f"Anomaly detection keys: {keys}")