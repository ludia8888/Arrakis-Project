        ...
        try:
            ...
            if self.jwks_client:
                try:
                    ...
                    key = signing_key.key
                    algorithms = ["RS256"]
                except (ValueError, KeyError, jwt.PyJWKClientError):
                    # Fallback to HS256
                    key = self.secret_key
                    algorithms = ["HS256"]
            else:
...
class IAMServiceClientWithFallback:
    ...
    async def validate_token(self, token: str, required_scopes: Optional[List[str]] = None) -> TokenValidationResponse:
        ...
        try:
            ...
            logger.error(f"Runtime error in token validation: {e}")
            iam_fallback_counter.labels(reason="runtime_error").inc()
    ...
    async def get_user_info(self, user_id: str) -> Optional[UserInfoResponse]:
        ...
        try:
            ...
            logger.error(f"Runtime error getting user info for {user_id}: {e}", exc_info=True)
            self._record_failure()
            raise IAMServiceError(f"Runtime IAM service error: {type(e).__name__}") from e
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Runtime health check error: {e}", exc_info=True)
            health_status["last_error"] = f"Runtime error: {type(e).__name__}"
