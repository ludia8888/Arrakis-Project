...
class PolicyEngine:
    ...
    def __init__(self, context: ExecutionContext, config_source: Optional[str]=None, runtime_overrides: Optional[Dict[str, Any]]=None):
        self.context = context
        self.config = self._load_policy_config(config_source, runtime_overrides)
        self.execution_stats = {'rules_processed': 0, 'actions_taken': {action.value: 0 for action in PolicyAction}, 'failures': [], 'warnings': [], 'alerts': []}
        self._custom_handlers: Dict[str, callable] = {}
        self._register_default_handlers()

    def _load_policy_config(self, config_source: Optional[str], runtime_overrides: Optional[Dict[str, Any]]) -> PolicyConfig:
    ...
    def _get_default_policy_config(self, context: ExecutionContext) -> PolicyConfig:
        ...
        if context == ExecutionContext.CI_BUILD:
            return PolicyConfig(context=context, default_action=PolicyAction.FAIL, fail_fast=True, max_warnings=10, max_alerts=5, rules=[PolicyRule('*', Severity.CRITICAL, PolicyAction.FAIL), PolicyRule('*', Severity.HIGH, PolicyAction.FAIL), PolicyRule('*', Severity.MEDIUM, PolicyAction.WARN), PolicyRule('*', Severity.LOW, PolicyAction.IGNORE)])
        elif context == ExecutionContext.CI_PR:
            return PolicyConfig(context=context, default_action=PolicyAction.WARN, fail_fast=False, max_warnings=50, max_alerts=20, rules=[PolicyRule('*', Severity.CRITICAL, PolicyAction.FAIL), PolicyRule('*', Severity.HIGH, PolicyAction.WARN), PolicyRule('*', Severity.MEDIUM, PolicyAction.WARN), PolicyRule('*', Severity.LOW, PolicyAction.IGNORE)])
        elif context == ExecutionContext.RUNTIME_BATCH:
            return PolicyConfig(context=context, default_action=PolicyAction.ALERT, fail_fast=False, max_warnings=1000, max_alerts=100, rules=[PolicyRule('*', Severity.CRITICAL, PolicyAction.FAIL), PolicyRule('*', Severity.HIGH, PolicyAction.ALERT), PolicyRule('*', Severity.MEDIUM, PolicyAction.WARN), PolicyRule('*', Severity.LOW, PolicyAction.IGNORE)])
        elif context == ExecutionContext.RUNTIME_STREAM:
            return PolicyConfig(context=context, default_action=PolicyAction.ALERT, fail_fast=False, max_warnings=10000, max_alerts=1000, rules=[PolicyRule('*', Severity.CRITICAL, PolicyAction.ALERT), PolicyRule('*', Severity.HIGH, PolicyAction.ALERT), PolicyRule('*', Severity.MEDIUM, PolicyAction.WARN), PolicyRule('*', Severity.LOW, PolicyAction.IGNORE)])
        elif context == ExecutionContext.PRODUCTION:
            return PolicyConfig(context=context, default_action=PolicyAction.ALERT, fail_fast=False, max_warnings=500, max_alerts=50, enable_notifications=True, rules=[PolicyRule('*', Severity.CRITICAL, PolicyAction.ALERT), PolicyRule('*', Severity.HIGH, PolicyAction.ALERT), PolicyRule('*', Severity.MEDIUM, PolicyAction.WARN), PolicyRule('*', Severity.LOW, PolicyAction.WARN)])
        else:
            return PolicyConfig(context=context, default_action=PolicyAction.WARN, fail_fast=False, max_warnings=1000, max_alerts=100, rules=[PolicyRule('*', Severity.CRITICAL, PolicyAction.WARN), PolicyRule('*', Severity.HIGH, PolicyAction.WARN), PolicyRule('*', Severity.MEDIUM, PolicyAction.WARN), PolicyRule('*', Severity.LOW, PolicyAction.IGNORE)])
    ...
    def _merge_configs(self, base: PolicyConfig, override: PolicyConfig) -> PolicyConfig:
        ...
        merged = PolicyConfig(context=base.context, default_action=override.default_action if override.default_action != PolicyAction.WARN else base.default_action, rules=override.rules if override.rules else base.rules, fail_fast=override.fail_fast if hasattr(override, 'fail_fast') else base.fail_fast, max_warnings=override.max_warnings if hasattr(override, 'max_warnings') else base.max_warnings, max_alerts=override.max_alerts if hasattr(override, 'max_alerts') else base.max_alerts)
        return merged
    ...
    def apply_policy(self, rule_results: List[RuleResult]) -> Dict[str, Any]:
        ...
        policy_result = {'should_fail': False, 'should_continue': True, 'actions_taken': [], 'summary': {'total_rules': len(rule_results), 'failures': 0, 'warnings': 0, 'alerts': 0, 'ignored': 0}, 'messages': []}
        for rule_result in rule_results:
            ...
            if not policy_result['should_continue']:
                break
        if self.execution_stats['actions_taken']['warn'] > self.config.max_warnings or self.execution_stats['actions_taken']['alert'] > self.config.max_alerts:
            policy_result['should_fail'] = True
            policy_result['messages'].append(f"Exceeded policy thresholds: warnings={self.execution_stats['actions_taken']['warn']}, alerts={self.execution_stats['actions_taken']['alert']}")
    ...
    def _execute_action(self, action: PolicyAction, rule_result: RuleResult, breaking_change: BreakingChange) -> Dict[str, Any]:
        """Execute policy action and return result"""
        rule_id = rule_result.metadata.get('rule_id', 'unknown_rule')
        action_result = {'action': action.value, 'rule_id': rule_id, 'severity': breaking_change.severity.value, 'description': breaking_change.description, 'timestamp': datetime.utcnow().isoformat()}
    ...
    def _find_custom_handler(self, rule_id: str) -> Optional[str]:
        ...
        for policy_rule in self.config.rules:
            if self._matches_pattern(rule_id, policy_rule.rule_pattern) and policy_rule.action == PolicyAction.CUSTOM:
    ...
    def _register_default_handlers(self):
        ...
        def foundry_compliance_handler(rule_result: RuleResult, breaking_change: BreakingChange) -> Dict[str, Any]:
            ...
            return {'custom_action': 'foundry_compliance_check', 'compliance_status': 'violation_detected', 'remediation_required': True}
        ...
        def performance_degradation_handler(rule_result: RuleResult, breaking_change: BreakingChange) -> Dict[str, Any]:
            ...
            return {'custom_action': 'performance_analysis', 'monitoring_enabled': True, 'auto_scaling_triggered': True}
        self._custom_handlers.update({'foundry_compliance': foundry_compliance_handler, 'performance_degradation': performance_degradation_handler})
    ...
    def get_execution_stats(self) -> Dict[str, Any]:
        ...
        return {**self.execution_stats, 'config_context': self.context.value, 'total_actions': sum(self.execution_stats['actions_taken'].values())}
...
def create_policy_engine_from_env() -> PolicyEngine:
    ...
    if unified_env.get('CI') == 'true':
        if unified_env.get('GITHUB_EVENT_NAME') == 'pull_request':
            context = ExecutionContext.CI_PR
        else:
            context = ExecutionContext.CI_BUILD
    elif unified_env.get('ENVIRONMENT') == 'production':
        context = ExecutionContext.PRODUCTION
    elif unified_env.get('RUNTIME_MODE') == 'batch':
        context = ExecutionContext.RUNTIME_BATCH
    elif unified_env.get('RUNTIME_MODE') == 'stream':
        context = ExecutionContext.RUNTIME_STREAM
    overrides = {}
    if unified_env.get('POLICY_FAIL_FAST'):
        overrides['fail_fast'] = unified_env.get('POLICY_FAIL_FAST').lower() == 'true'
    if unified_env.get('POLICY_MAX_WARNINGS'):
        overrides['max_warnings'] = int(unified_env.get('POLICY_MAX_WARNINGS'))
    if unified_env.get('POLICY_MAX_ALERTS'):
        overrides['max_alerts'] = int(unified_env.get('POLICY_MAX_ALERTS'))
    if unified_env.get('POLICY_DEFAULT_ACTION'):
        overrides['default_action'] = unified_env.get('POLICY_DEFAULT_ACTION')
    config_file = unified_env.get('POLICY_CONFIG_FILE')
    return PolicyEngine(context=context, config_source=config_file, runtime_overrides=overrides if overrides else None)

def create_ci_policy_engine(fail_fast: bool=True) -> PolicyEngine:
    """Create policy engine for CI/CD environments"""
    return PolicyEngine(context=ExecutionContext.CI_BUILD, runtime_overrides={'fail_fast': fail_fast})
...
def create_production_policy_engine(enable_notifications: bool=True) -> PolicyEngine:
    ...
    config = PolicyConfig(context=ExecutionContext.PRODUCTION, enable_notifications=enable_notifications)
