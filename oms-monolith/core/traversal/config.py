...
class TraversalConfig:
    ...
    max_entities_to_analyze: int = 50
    max_high_degree_nodes: int = 10
    common_entities_conflict_threshold: int = 10
    entity_properties_threshold: int = 5
    high_impact_change_threshold: int = 10
    merge_complexity_threshold: int = 10
    ...
    msa_service_mappings: Dict[str, str] = field(default_factory=lambda: {'Order': 'order-service', 'Product': 'product-service', 'Customer': 'customer-service', 'Inventory': 'inventory-service', 'Payment': 'payment-service', 'User': 'user-service', 'Notification': 'notification-service', 'Analytics': 'analytics-service'})
    msa_critical_services: List[str] = field(default_factory=lambda: ['user-service', 'order-service', 'payment-service'])
    msa_high_impact_threshold: int = 3
...
class ConfigManager:
    ...
    def _load_traversal_config(self) -> TraversalConfig:
        ...
        config.max_traversal_depth = int(unified_env.get('TRAVERSAL_MAX_DEPTH'))
        config.default_branch = unified_env.get('TRAVERSAL_DEFAULT_BRANCH')
        return config
...
def get_config(env: str=None) -> ConfigManager:
    ...
    if _config_manager is None or (env and _config_manager.env != env):
        _config_manager = ConfigManager(env or unified_env.get('ENVIRONMENT'))
