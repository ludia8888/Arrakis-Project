...
class CodeQualityAnalyzer:
    ...
    def analyze_file(self, file_path: str) -> List[str]:
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                # E501: Line too long
                if len(line) > 88:
                    issues.append(f"{file_path}:{i}: E501 line too long ({len(line)} > 88)")
                ...
                if line.strip() == '' and len(line) > 0:
                    issues.append(f"{file_path}:{i}: W293 blank line contains whitespace")
                
                # E225: Missing whitespace around operator
                if '=' in line and '==' not in line and '!=' not in line and '<=' not in line and '>=' not in line:
                    # Simple check for assignment operators
                    if re.search(r'[a-zA-Z0-9_]\=[a-zA-Z0-9_]', line):
                        issues.append(f"{file_path}:{i}: E225 missing whitespace around operator")
                ...
                if line.strip().startswith('class ') or line.strip().startswith('def ') and not line.strip().startswith('def _'):
                    if i > 2:
                        ...
                        if not all(l == '' for l in prev_lines[-2:]):
                            issues.append(f"{file_path}:{i}: E302 expected 2 blank lines")
                
                # E261: At least two spaces before inline comment
                if '#' in line and not line.strip().startswith('#'):
                    comment_pos = line.find('#')
                    if comment_pos > 0 and not line[comment_pos-2:comment_pos] == '  ':
                        issues.append(f"{file_path}:{i}: E261 at least two spaces before inline comment")
            ...
            for i, line in enumerate(lines, 1):
                if line.strip().startswith('import '):
                    ...
                    if module not in ['os', 'sys', 'json', 'logging'] and content.count(module) <= 1:
                        issues.append(f"{file_path}:{i}: F401 '{module}' imported but unused")
    ...
    def analyze_directory(self, directory: str, exclude_dirs: List[str] = None) -> Dict[str, List[str]]:
        """Analyze all Python files in a directory"""
        if exclude_dirs is None:
            exclude_dirs = ['__pycache__', '.git', 'venv', 'node_modules', 'htmlcov', 'logs', '.pytest_cache']
