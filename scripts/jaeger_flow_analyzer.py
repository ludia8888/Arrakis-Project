...
class JaegerFlowAnalyzer:
    ...
    def _analyze_service_dependencies(self, traces: List[Dict]) -> Dict:
        ...
        for service in self.ARRAKIS_SERVICES:
            ...
            service_stats[service] = {
                ...
                'fan_in': len([s for s, deps in dependency_map.items() if service in deps]),
        ...
        return {
            ...
            'most_called_services': sorted(
                [(service, stats['incoming_calls']) for service, stats in service_stats.items()],
                key=lambda x: x[1],
                reverse=True
            )[:10]
        }
    ...
    def _analyze_flow_patterns(self, traces: List[Dict]) -> Dict:
        ...
        common_patterns = sorted(flow_patterns.items(), key=lambda x: x[1], reverse=True)[:20]
    ...
    def _identify_hotpaths(self, traces: List[Dict]) -> Dict:
        ...
        critical_paths.sort(key=lambda x: (x['frequency'], x['avg_duration']), reverse=True)
        
        return {
            'hot_paths': critical_paths[:20],
            'path_frequencies': dict(path_frequencies),
            'most_frequent_paths': sorted(path_frequencies.items(), key=lambda x: x[1], reverse=True)[:10]
        }
    ...
    async def _generate_performance_dashboard(self, performance: Dict, output_dir: Path):
    ...
    async def _generate_error_visualization(self, error_analysis: Dict, output_dir: Path):
    ...
    async def _generate_interactive_dashboard(self, analysis_results: Dict, output_dir: Path):
        ...
        for service, calls in analysis_results['service_dependencies']['most_called_services'][:5]:
        ...
        sorted_perf = sorted(performance.items(), key=lambda x: x[1]['mean_duration'], reverse=True)
