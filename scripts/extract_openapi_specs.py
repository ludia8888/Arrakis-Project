...
class OpenAPIExtractor:
    ...
    def _enhance_openapi_spec(self, spec: Dict, service: ServiceConfig):
        ...
        spec["info"]["x-docs-url"] = f"{service.url}:{service.port}{service.docs_endpoint}"
...
'''
    ...
    async def extract_all_specs(self, check_health: bool = True):
        ...
        if extracted_specs:
            ...
            logger.info(f"🎉 Successfully extracted {len(extracted_specs)} OpenAPI specifications!")
...
async def main():
    parser = argparse.ArgumentParser(description="Extract OpenAPI specs from Arrakis services")
    parser.add_argument("--output-dir", default="docs/openapi", 
