...
class ArchitectureDiagramGenerator:
                ...
                for key, value in env_vars.items():
                    if '_URL' in key or '_ENDPOINT' in key:
                        ...
                        if target_service and target_service in self.services:
                            dep_type = self._determine_dependency_type_from_env(key, value)
                            self.dependencies.append(ServiceDependency(
                                from_service=service_name,
    ...
    def generate_system_overview_diagram(self) -> str:
        ...
        for service in app_services:
            ...
            tech_info = f"<br/>{', '.join(service.tech_stack[:2])}" if service.tech_stack else ""
            mermaid.append(f"        {service.name}[\"{icon} {service.name.replace('-', ' ').title()}{tech_info}\"]")
        ...
        # Add data services
        data_services = [s for s in self.services.values() if s.type in ['database', 'cache']]
        ...
        infra_services = [s for s in self.services.values() if s.type in ['messaging', 'monitoring']]
    ...
    def generate_service_dependency_diagram(self) -> str:
        ...
        for service in self.services.values():
            if service.type == 'service':
                apis_info = f"<br/>APIs: {', '.join(service.apis[:2])}" if service.apis else ""
                port_info = f"<br/>Port: {service.port}" if service.port else ""
                mermaid.append(f"    {service.name}[\"{service.name.replace('-', ' ').title()}{apis_info}{port_info}\"]")
    ...
    def generate_data_flow_diagram(self) -> str:
        ...
        for service_name in core_services:
            if service_name in self.services:
                ...
                mermaid.append(f"    {service_name}[\"{icon} {service.name.replace('-', ' ').title()}\"]")
    ...
    def generate_technology_stack_diagram(self) -> str:
        ...
        for service in python_services:
            ...
            mermaid.append(f"        {service.name}[\"{service.name.replace('-', ' ').title()}<br/>{apis}\"]")
    ...
    def save_diagrams(self, diagrams: Dict[str, str]):
        ...
        logger.info(f"💾 Saved README.md with all diagrams")
