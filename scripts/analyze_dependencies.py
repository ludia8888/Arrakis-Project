...
class DependencyAnalyzer:
    ...
    def analyze_service(self, service: str, analysis_types: List[str] = None) -> Dict:
        ...
        if "pyreverse" in analysis_types:
            results["analyses"]["pyreverse"] = self._run_pyreverse(
                service_path, output_path
            )

        if "imports" in analysis_types:
            results["analyses"]["imports"] = self._analyze_imports(
                service_path, output_path
            )

        if "metrics" in analysis_types:
            results["analyses"]["metrics"] = self._calculate_metrics(
                service_path, output_path
            )
                ...
                if module_path.exists() and module_path.is_dir():
                    ...
                    if output_file.exists():
                        results["graphs"].append(
                            str(output_file.relative_to(self.output_dir))
                        )
    ...
    def _analyze_imports(self, service_path: Path, output_path: Path) -> Dict:
        ...
        for py_file in service_path.rglob("*.py"):
            ...
            try:
                ...
                module_name = (
                    str(py_file.relative_to(service_path))
                    .replace("/", ".")
                    .replace(".py", "")
                )
                imports = []
        ...
        for module, imports in import_graph.items():
            ...
            for imp in imports:
                ...
                if imp.startswith(internal_prefix) or module.startswith(
                    imp.split(".")[0]
                ):
    ...
    def _calculate_metrics(self, service_path: Path, output_path: Path) -> Dict:
        ...
        try:
            vulture_output = subprocess.run(
                ["vulture", str(service_path), "--min-confidence", "80"],
                capture_output=True,
                text=True,
            )
        ...
        try:
            bandit_output = subprocess.run(
                ["bandit", "-r", str(service_path), "-", "json"],
                capture_output=True,
                text=True,
            )
...
"""
    ...
    def _generate_combined_report(self, results: Dict[str, Dict]):
        ...
        with open(report_path, "w") as f:
            f.write(
                """# Arrakis Project - Dependency Analysis Dashboard
...
"""
            ...
            for service, result in results.items():
                ...
                f.write(
                    f"| [{service}](./{service}/) | {status} | {complexity} | {security} | {deps} |\n"
                )
...
def main():
    ...
    parser.add_argument(
        "--output", default="docs/dependencies", help="Output directory for reports"
    )
    parser.add_argument(
        "--parallel", action="store_true", help="Run analysis in parallel"
    )
    parser.add_argument(
        "--types",
        nargs="+",
    ...
    if args.service == "all":
        ...
        logger.info(
            f"Analysis complete for {args.service}. Results saved to {args.output}/{args.service}"
        )
