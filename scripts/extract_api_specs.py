...
Automatically extracts OpenAPI specs, AsyncAPI specs, and event schemas from deployments.
"""
...
class APISpecExtractor:
    ...
    async def _get_service_url(self, service_name: str, service_config: Dict) -> Optional[str]:
    ...
    async def _get_k8s_service_url(self, service_name: str, service_config: Dict) -> Optional[str]:
    ...
    async def _generate_openapi_docs(self, spec: Dict, output_dir: Path):
        ...
        with open(docs_file, 'w') as f:
            ...
            f.write(f"**Description:** {info.get('description', 'No description available')}\n\n")
            
            if 'servers' in spec:
                f.write("## Servers\n\n")
                for server in spec['servers']:
                    f.write(f"- {server.get('url', 'N/A')}: {server.get('description', 'No description')}\n")
                f.write("\n")
                    ...
                    if method.upper() in ['GET', 'POST', 'PUT', 'DELETE', 'PATCH']:
                        ...
                        if 'parameters' in details:
                            ...
                            for param in details['parameters']:
                                f.write(f"- `{param.get('name', 'unknown')}` ({param.get('in', 'unknown')}): {param.get('description', 'No description')}\n")
                            f.write("\n")
    ...
    async def _generate_service_docs(self, service_name: str, result: Dict, 
        ...
        with open(docs_file, 'w') as f:
            ...
            if 'openapi' in specs:
                ...
                f.write(f"### OpenAPI Specification\n")
                ...
                f.write(f"- **Documentation:** [openapi.md](openapi.md)\n\n")
            
            if 'asyncapi' in specs:
                asyncapi = specs['asyncapi']
                f.write(f"### AsyncAPI Specification\n")
    ...
    def _generate_markdown_report(self, report: Dict):
        ...
        with open(report_file, 'w') as f:
            f.write(f"# API Specification Extraction Report\n\n")
            ...
            f.write("2. **Testing:** Import specs into Postman, Insomnia, or other API testing tools\n")
            f.write("3. **Documentation:** Upload to documentation portals (Redocly, SwaggerHub)\n")
