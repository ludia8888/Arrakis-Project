...
class EventGatewayService:
    ...
    async def subscribe(
        ...
        for subject in subjects:
            sub = await self.js.pull_subscribe(
                subject, durable=durable_name, config=config
            )
            subs.append(sub)
    ...
    async def stream_events(
        self, subscription_id: str, batch_size: int = 10, timeout_seconds: int = 1
    ) -> AsyncIterator[Event]:
    ...
    async def acknowledge_event(
        self, consumer_id: str, event_id: str, sequence: int
    ) -> bool:
    ...
    async def register_webhook(self, webhook: Webhook) -> Webhook:
        ...
        # Save to Redis
        await self.redis.hset("webhooks", webhook.id, webhook.json())

        logger.info(f"Registered webhook: {webhook.id}")
        return webhook

    async def update_webhook(
        self, webhook_id: str, webhook: Webhook
    ) -> Optional[Webhook]:
    ...
    async def list_webhooks(
        self, event_types: Optional[List[str]] = None, enabled_only: bool = False
    ) -> List[Webhook]:
    ...
    async def create_stream(self, stream: Stream) -> Stream:
        ...
        config = {
            ...
            "max_age": (
                stream.config.max_age_seconds * 1_000_000_000
                if stream.config.max_age_seconds
                else 0
            ),
        ...
        for info in stream_names:
            stream = Stream(
                ...
                config=StreamConfig(
                    ...
                    max_age_seconds=(
                        info.config.max_age // 1_000_000_000
                        if info.config.max_age
                        else 0
                    ),
                ...
                created_at=datetime.now(
                    timezone.utc
                ),  # NATS doesn't track creation time
            )
    ...
    async def _create_default_streams(self):
        ...
        default_streams = [
            Stream(
                ...
                subjects=["events.>"],
                config=StreamConfig(max_age_seconds=86400 * 7, replicas=1),  # 7 days
            ),
            Stream(
                name="audit",
                description="Audit event stream",
                subjects=["audit.>"],
                config=StreamConfig(max_age_seconds=86400 * 30, replicas=1),  # 30 days
            ),
            Stream(
                name="metrics",
                description="Metrics event stream",
                subjects=["metrics.>"],
                config=StreamConfig(max_age_seconds=86400, replicas=1),  # 1 day
            ),
        ]
...
class WebhookExecutor:
    ...
    async def deliver(self, webhook: Webhook, event_dict: Dict) -> bool:
        ...
        async with httpx.AsyncClient(
            timeout=webhook.config.timeout_seconds, verify=webhook.config.verify_ssl
        ) as client:
            # Prepare headers
            ...
            if webhook.config.secret:
                signature = self._calculate_signature(
                    webhook.config.secret, json.dumps(event_dict)
                )
                headers["X-Webhook-Signature"] = signature

            # Send request
            response = await client.post(webhook.url, json=event_dict, headers=headers)
