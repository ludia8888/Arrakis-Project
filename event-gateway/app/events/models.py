...
class Webhook(BaseModel):
    ...
    event_types: List[str] = Field(
        default_factory=list, description="Event types to deliver"
    )
...
class StreamConfig(BaseModel):
    ...
    max_age_seconds: int = Field(
        default=0, description="Maximum age in seconds, 0 for unlimited"
    )
    max_msg_size: int = Field(
        default=-1, description="Maximum message size, -1 for unlimited"
    )
    replicas: int = Field(default=1, description="Number of replicas")
    discard_new_per_subject: bool = Field(
        default=False, description="Discard new messages when limit reached"
    )
