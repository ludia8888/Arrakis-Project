...
events_published_counter = Counter(
    "event_gateway_events_published_total", "Total events published", ["stream", "type"]
)
events_delivered_counter = Counter(
    "event_gateway_events_delivered_total", "Total events delivered", ["consumer"]
)
webhook_deliveries_counter = Counter(
    "event_gateway_webhook_deliveries_total",
    "Total webhook deliveries",
    ["webhook", "status"],
)
event_processing_histogram = Histogram(
    "event_gateway_processing_seconds", "Event processing time"
)
...
async def publish_event(
    ...
    try:
        ...
        result = await service.publish_event(
            cloud_event, request.stream, request.headers
        )

        # Update metrics
        events_published_counter.labels(
            stream=request.stream, type=cloud_event["type"]
        ).inc()
...
async def publish_events_batch(
    ...
    try:
        ...
        results = await service.publish_events_batch(
            cloud_events, request.stream, request.headers
        )

        # Update metrics
        for event in cloud_events:
            events_published_counter.labels(
                stream=request.stream, type=event["type"]
            ).inc()
...
async def create_subscription(
    request: SubscribeRequest, service: EventGatewayService = Depends(get_event_service)
):
    ...
    async def event_generator():
        try:
            async for event in service.stream_events(subscription_id, batch_size):
                ...
                # Update metrics
                events_delivered_counter.labels(consumer=subscription_id).inc()
    ...
    try:
        async for event in service.stream_events(subscription_id):
            await websocket.send_json(
                {"event": event.cloud_event, "metadata": event.metadata.model_dump()}
            )
...
async def get_webhook(
    webhook_id: str, service: EventGatewayService = Depends(get_event_service)
):
    """Get a webhook by ID."""
...
async def delete_stream(
    stream_name: str, service: EventGatewayService = Depends(get_event_service)
):
    """Delete a stream."""
...
async def list_streams(service: EventGatewayService = Depends(get_event_service)):
    """List all streams."""
