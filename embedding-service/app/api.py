...
async def generate_embedding(
    ...
    try:
        ...
        embedding = await service.generate_embedding(
            request.text, metadata=request.metadata
        )
...
async def generate_batch_embeddings(
    ...
    try:
        ...
        embeddings = await service.generate_batch_embeddings(
            request.texts, metadata=request.metadata
        )
...
async def calculate_similarity(
    ...
    try:
        ...
        similarity = await service.calculate_similarity(
            embedding1, embedding2, metric=request.metric
        )

        return SimilarityResponse(
            similarity=float(similarity), metric_used=request.metric
        )
    except Exception as e:
