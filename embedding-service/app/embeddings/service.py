...
embedding_request_duration = Histogram(
 "embedding_request_duration_seconds", "Embedding request duration", ["provider"]
)
embedding_cache_hits = Counter(
 "embedding_cache_hits_total", "Embedding cache hits", ["provider"]
)
embedding_cache_misses = Counter(
 "embedding_cache_misses_total", "Embedding cache misses", ["provider"]
)
embedding_batch_size = Histogram(
 "embedding_batch_size", "Embedding batch sizes", ["provider"]
)
active_embedding_providers = Gauge(
 "active_embedding_providers", "Number of active embedding providers"
)
...
class VectorEmbeddingService:
 ...
 self, name: str, config: EmbeddingConfig, is_default: bool = False
 ):
 ...
 provider = provider_name or "unknown", status = "error"
 ).inc()
 ...
 embedding_requests_total.labels(
 provider = provider_name, status = "token_limit_error"
 ).inc()
 ...
 embedding_requests_total.labels(
 provider = provider_name, status = "api_error"
 ).inc()
 ...
 embedding_requests_total.labels(
 provider = provider_name, status = "unknown_error"
 ).inc()
 ...
 self, embedding_ids: List[str], n_clusters: int = 5, algorithm: str = "kmeans"
 ) -> Dict[str, Any]:
 """
 ...
 "predicate": {
 "@type": "woql:Node",
 "node": f"embedding:meta_{key}",
 },
 "object": {
 "@type": "woql:DataValue",
 "data": {"@type": "xsd:string", "@value": str(value)},
 },
 }
 )
 ...
 # Add limit
 if top_k > 0:
 final_query = {"@type": "woql:Limit", "query": final_query, "limit": top_k}
 ...
 self, texts: List[str], provider_name: str
 ...
 self, text: str, provider_name: str, embedding_doc: Dict[str, Any]
 ):
 ...
 stats = {"providers": {}, "total_embeddings": 0, "cache_stats": {}}
 ...
 self, texts: List[str], preferred_provider: str
 ) -> List[List[float]]:
 ...
 try:
 logger.info(
 f"Attempting to create embeddings with provider: {provider_name}"
 )
 ...
 last_error = e
 logger.error(
 f"Provider {provider_name} failed with unexpected error: {e}"
 )
 continue
