...
# Production ML dependencies - required for embedding providers
...
class BaseEmbeddingProvider(ABC):
    ...
    def _split_into_batches(self, texts: List[str]) -> List[List[str]]:
        ...
        for text in texts:
            ...
            if current_batch and (
                ...
                or current_tokens + estimated_tokens > max_tokens
            ):
    ...
    async def create_embeddings_with_batching(
        self, texts: List[str]
    ) -> List[List[float]]:
    ...
    async def _check_rate_limit(self) -> None:
        ...
        async with self._rate_limit_lock:
            ...
            if request_count >= self.config.rate_limit_rpm:
                ...
                if wait_time > 0:
                    logger.warning(
                        f"Rate limit reached for {provider_key}, waiting {wait_time:.2f}s"
                    )
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "rate_limit" in str(e).lower():
                    ...
                    logger.warning(
                        f"Rate limit error, attempt {attempt + 1}/{max_retries}, waiting {wait_time}s"
                    )
                    await asyncio.sleep(wait_time)
                else:
                    ...
                    logger.warning(
                        f"API error, attempt {attempt + 1}/{max_retries}, waiting {wait_time}s"
                    )
                    await asyncio.sleep(wait_time)
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"Unexpected error, attempt {attempt + 1}/{max_retries}, waiting {wait_time}s"
                    )
                    await asyncio.sleep(wait_time)
                else:
                    break
    ...
    async def similarity_search(
        ...
        for idx, doc_embedding in enumerate(document_embeddings):
            ...
            similarities.append({"index": idx, "similarity": float(similarity)})
        ...
        try:
            ...
            if "rate_limit_exceeded" in error_message.lower():
                ...
                raise EmbeddingTokenLimitError(
                    f"OpenAI token limit exceeded: {error_message}"
                )
            else:
    ...
    def _split_into_batches(self, texts: List[str]) -> List[List[str]]:
        ...
        for text in texts:
            ...
            if text_tokens > max_tokens:
                logger.warning(
                    f"Text exceeds token limit ({text_tokens} > {max_tokens}), truncating"
                )
...
class CohereEmbeddingProvider(BaseEmbeddingProvider):
    ...
    async def create_embeddings(self, texts: List[str]) -> List[List[float]]:
        response = await self.client.embed(
            texts=texts, model=self.config.model_name, input_type="search_document"
        )
...
class AnthropicEmbeddingProvider(BaseEmbeddingProvider):
    ...
    def _init_fallback_provider(self):
        ...
        logger.info(
            "Anthropic provider using local Sentence Transformers for embeddings"
        )
