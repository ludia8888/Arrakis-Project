...
class StartupOptimizer:
    ...
    async def optimize_startup(self):
        ...
        logger.info(
            f"Service ready in {critical_time:.2f}s, background tasks scheduled"
        )
    ...
    async def _run_background_task(self, task: Dict[str, Any]):
        ...
        try:
            ...
            duration = time.time() - start
            logger.info(
                f"Background task '{task['name']}' completed in {duration:.2f}s"
            )
...
class EmbeddingServiceOptimizer(StartupOptimizer):
    ...
    def _setup_tasks(self):
        ...
        self.add_critical_task(
            "initialize_default_model", self._initialize_default_model
        )

        self.add_critical_task("connect_redis", self._connect_redis)

        # Background tasks - can be done after service is ready
        self.add_background_task("preload_popular_models", self._preload_popular_models)

        self.add_background_task("warm_cache", self._warm_cache)
    ...
    async def _warm_cache(self):
        ...
        test_texts = ["Hello world", "Test embedding", "Sample document"]
...
class SchedulerServiceOptimizer(StartupOptimizer):
    ...
    def _setup_tasks(self):
        ...
        self.add_critical_task("start_scheduler", self._start_scheduler)

        # Background tasks
        self.add_background_task("restore_jobs", self._restore_jobs)
...
class EventGatewayOptimizer(StartupOptimizer):
    ...
    def _setup_tasks(self):
        ...
        # Critical tasks
        self.add_critical_task("connect_nats", self._connect_nats)

        self.add_critical_task("connect_redis", self._connect_redis)

        # Background tasks
        self.add_background_task("create_default_streams", self._create_default_streams)

        self.add_background_task(
            "start_webhook_processor", self._start_webhook_processor
        )
    ...
    async def _connect_redis(self):
        ...
        self.service.redis = await aioredis.from_url(
            self.service.redis_url, decode_responses=True
        )
