        ...
        try:
            ...
            return JSONResponse(
                ...
                headers={
                    ...
                    "X-RateLimit-Remaining": "0",
                    "X-RateLimit-Reset": str(int(e.reset_time.timestamp()))
                    if e.reset_time
                    else "",
                },
            )
...
def rate_limit_ip(limit: int, window: int = 60, **kwargs):
    ...
    return rate_limit_decorator(
        limit=limit, window=window, scope=RateLimitScope.IP, **kwargs
    )
...
def rate_limit_user(limit: int, window: int = 60, **kwargs):
    """Rate limit by user"""
    return rate_limit_decorator(
        limit=limit, window=window, scope=RateLimitScope.USER, **kwargs
    )
...
def rate_limit_endpoint(limit: int, window: int = 60, **kwargs):
    ...
    return rate_limit_decorator(
        limit=limit, window=window, scope=RateLimitScope.ENDPOINT, **kwargs
    )
