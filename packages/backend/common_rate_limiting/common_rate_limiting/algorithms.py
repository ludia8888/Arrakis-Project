...
class SlidingWindowAlgorithm(RateLimitAlgorithm):
    ...
    async def is_allowed(
        ...
        valid_requests = [req for req in requests if req["timestamp"] > window_start]
        ...
        if allowed:
            valid_requests.append({"timestamp": current_time, "cost": cost})
            remaining = limit - (current_count + cost)
        else:
            remaining = 0
        ...
        if valid_requests:
            ...
            retry_after = (
                int(oldest_timestamp + window - current_time) if not allowed else None
            )
        else:
...
class TokenBucketAlgorithm(RateLimitAlgorithm):
    ...
    async def is_allowed(
        ...
        new_state = {"tokens": new_tokens, "last_refill": current_time}
...
class LeakyBucketAlgorithm(RateLimitAlgorithm):
    ...
    async def is_allowed(
        ...
        new_state = {"queue_size": new_queue_size, "last_leak": current_time}
