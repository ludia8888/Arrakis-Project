...
class EventPublisher:
 ...
 "EventPublisher is an abstract interface - use concrete implementation"
 )
...
class EventSubscriber:
 ...
 raise NotImplementedError(
 "EventSubscriber is an abstract interface - use concrete implementation"
 )
...
class OMSEventClient:
 ...
 nats_url = config.nats_url or os.getenv(
 "OMS_NATS_URL", "nats://nats.oms.company.com:4222"
 )
 ws_url = config.websocket_url or os.getenv(
 "OMS_WEBSOCKET_URL", "ws://localhost:8080"
 )
 ...
 from .nats_adapter import NATSPublisher, NATSSubscriber
 ...
 return cls(publisher, subscriber)

 except ImportError:
 # Fallback to in-memory implementation for testing
 from .memory_adapter import InMemoryPublisher, InMemorySubscriber

 publisher = InMemoryPublisher()
 subscriber = InMemorySubscriber(publisher)
