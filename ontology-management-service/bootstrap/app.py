...
def create_app(config: Optional[AppConfig] = None) -> FastAPI:
 ...
 logger.warning(
 f"Failed to initialize Redis client and Global Circuit Breaker: {e}"
 )
 ...
 logger.info(
 "🎯 Enterprise Observability Stack initialized successfully!"
 )
 ...
 logger.info(
 " 🚨 Memory leak detection - Automatic suspect identification"
 )
 ...
 logger.critical(
 f"Failed to initialize container resources: {e}", exc_info = True
 )
 raise
 ...
 and app.state.observability_manager
 ):
 ...
 if (
 hasattr(app.state, "pyroscope_profiler")
 and app.state.pyroscope_profiler
 ):
 try:
 ...
 # Cleanup Discovery
 if (
 hasattr(app.state, "discovery_coordinator")
 and app.state.discovery_coordinator
 ):
 try:
 ...
 logger.error(
 f"Error during container resource shutdown: {e}", exc_info = True
 )
 ...
 app.include_router(
 organization_routes.router, prefix = "/api/v1", tags = ["Organization"]
 )
 ...
 circuit_breaker_routes,
 config_routes
 # audit_routes - REMOVED: Moved to separate Audit Service
 # test_routes - REMOVED: No such module exists
