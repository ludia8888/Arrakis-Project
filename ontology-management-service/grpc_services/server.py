...
grpc_requests_total = Counter(
 "grpc_requests_total", "Total gRPC requests", ["service", "method", "status"]
)

grpc_request_duration = Histogram(
 "grpc_request_duration_seconds", "gRPC request duration", ["service", "method"]
)

grpc_active_connections = Gauge("grpc_active_connections", "Active gRPC connections")
...
class AuthInterceptor(grpc.ServerInterceptor):
 ...
 return self._create_error_handler(
 grpc.StatusCode.UNAUTHENTICATED, f"Invalid token: {str(e)}"
 )
 ...
 return UserContext(
 id = 1, username = "grpc_user", email = "grpc@example.com", roles = ["admin"]
 )
...
class SchemaServicer(schema_service_pb2_grpc.SchemaServiceServicer):
 ...
 branch = request.branch, schema_id = request.schema_id
 )
 ...
 "properties": [
 self._from_proto_property(p) for p in proto_schema.properties
 ],
 }
...
class EnterpriseGrpcServer:
 ...
 raise ValueError(
 "🚨 SECURITY ERROR: JWT_SECRET_KEY environment variable is required for gRPC authentication. "
 "No default secret allowed for production security."
 )
 ...
 SERVICE_NAMES = (
 schema_service_pb2.DESCRIPTOR.services_by_name[
 "SchemaService"
 ].full_name,
 branch_service_pb2.DESCRIPTOR.services_by_name[
 "BranchService"
 ].full_name,
...
class RequestTrackingInterceptor(grpc.ServerInterceptor):
 ...
 grpc_request_duration.labels(service = service, method = method).observe(
 duration
 )
