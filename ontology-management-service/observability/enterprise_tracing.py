...
class EnterpriseTracer:
 ...
 "JAEGER_COLLECTOR_ENDPOINT", self.config.jaeger_endpoint
 )
 ...
 logger.info(
 f"Enterprise tracing initialized for {self.config.service_name}"
 )
...
class ResilienceTracing:
 ...
 f"circuit_breaker.{operation}", kind = trace.SpanKind.INTERNAL
 ) as span:
 # 기본 속성 설정
 span.set_attributes(
 {
 ...
 span.set_attribute(
 "circuit_breaker.state", str(args[0].state)
 )
 if hasattr(args[0], "failure_count"):
 span.set_attribute(
 "circuit_breaker.failure_count", args[0].failure_count
 )
 ...
 span.set_attribute(
 "circuit_breaker.duration_ms", duration * 1000
 )
 ...
 async def wrapper(*args, **kwargs):
 with self.tracer.start_as_current_span(
 f"cache.{cache_type}.{operation}", kind = trace.SpanKind.INTERNAL
 ) as span:
 # 기본 속성 설정
 span.set_attributes(
 {
 ...
 if len(args) > 1:
 cache_key = (
 args[1] if isinstance(args[1], str) else str(args[1])
 )
 ...
 span.set_attribute(
 "cache.ttl_seconds", getattr(result, "ttl", 0)
 )
 ...
 async def wrapper(*args, **kwargs):
 with self.tracer.start_as_current_span(
 f"backpressure.{operation}", kind = trace.SpanKind.INTERNAL
 ) as span:
 span.set_attributes(
 {
 ...
 span.set_attribute(
 "backpressure.queue_size", args[0].queue_size
 )
 ...
 async def wrapper(*args, **kwargs):
 with self.tracer.start_as_current_span(
 f"http.{method.lower()}.{endpoint}", kind = trace.SpanKind.SERVER
 ) as span:
 # HTTP 관련 표준 속성
 span.set_attributes(
 {
 ...
 # 요청 시작 시간
 span.set_attribute(
 "http.request.start_time", datetime.utcnow().isoformat()
 )
 ...
 async def wrapper(*args, **kwargs):
 with self.tracer.start_as_current_span(
 f"db.{db_type}.{operation}", kind = trace.SpanKind.CLIENT
 ) as span:
 span.set_attributes(
 {
...
class BusinessTracing:
 ...
 async def wrapper(*args, **kwargs):
 with self.tracer.start_as_current_span(
 f"schema.{operation}", kind = trace.SpanKind.INTERNAL
 ) as span:
 span.set_attributes(
 {
 ...
 span.set_attribute(
 "schema.validation.valid", result.is_valid
 )
 if hasattr(result, "errors"):
 span.set_attribute(
 "schema.validation.error_count", len(result.errors)
 )
 ...
 async def wrapper(*args, **kwargs):
 with self.tracer.start_as_current_span(
 f"document.{operation}", kind = trace.SpanKind.INTERNAL
 ) as span:
 span.set_attributes(
 {
 ...
 async def wrapper(*args, **kwargs):
 with self.tracer.start_as_current_span(
 f"audit.{event_type}", kind = trace.SpanKind.INTERNAL
 ) as span:
 span.set_attributes(
 {
...
def setup_auto_instrumentation(app = None):
 """Production auto-instrumentation setup - all libraries must be available"""
 ...
 logger.info("Production auto-instrumentation setup completed")
