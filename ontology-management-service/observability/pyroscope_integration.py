...
# Pyroscope client availability check
try:
    import pyroscope
    HAS_PYROSCOPE = True
except ImportError:
    HAS_PYROSCOPE = False
    pyroscope = None
    logger.warning("Pyroscope client not installed. Install with: pip install pyroscope-io")

# py-spy profiler availability check (for more detailed profiling)
try:
    import py_spy
    HAS_PY_SPY = True
except ImportError:
...
class PyroscopeProfiler:
    ...
    def start_profiling(self) -> bool:
        ...
        if not HAS_PYROSCOPE:
            logger.error("Pyroscope client not available. Cannot start profiling.")
            return False
    ...
    def stop_profiling(self):
        """Stop profiling"""
        if HAS_PYROSCOPE and self.profiling_started:
            try:
                pyroscope.stop()
                self.profiling_started = False
...
def get_profiler_instance(config: Optional[PyroscopeConfig] = None) -> PyroscopeProfiler:
    ...
    def _setup_metrics(self):
        ...
        try:
            ...
            self.profiling_active = Gauge(
                'pyroscope_profiling_active',
                'Whether Pyroscope profiling is active'
            )
            
            self.profiling_info = Info(
                'pyroscope_profiling',
                'Pyroscope profiling information'
            )
            ...
            self.profile_upload_failures = Gauge(
                'pyroscope_profile_upload_failure_total',
                'Total failed profile uploads'
            )
