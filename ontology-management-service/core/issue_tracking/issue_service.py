...
class JiraClient(IssueProviderClient):
 ...
 self.email = email
 self.client = create_basic_client(base_url = self.base_url, timeout = 10.0)
 ...
 f"/rest/api/3/issue/{issue_id}", headers = self._auth_headers
 )
 ...
 return IssueValidationResult(
 valid = False, error_message = f"JIRA API error: {response.status_code}"
 )
 ...
 issue_ref = issue_ref,
 exists = True,
 status_valid = issue_ref.status
 in [IssueStatus.IN_PROGRESS, IssueStatus.IN_REVIEW],
 type_valid = True,
 age_valid = age_valid,
 issue_metadata={
 "key": data.get("key"),
 ...
 return IssueValidationResult(
 valid = False, error_message = f"Failed to validate issue: {str(e)}"
 )
 ...
 assignee = (
 metadata.get("fields", {}).get("assignee", {}).get("emailAddress")
 )
...
class InternalIssueClient(IssueProviderClient):
 """Internal issue tracking client - PostgreSQL Production Implementation"""

 def __init__(self):
 from core.issue_tracking.issue_database import get_issue_database

 self._db = None
 self._get_database = get_issue_database

 async def _ensure_database(self):
 """Ensure database connection is available"""
 if self._db is None:
 self._db = await self._get_database()

 async def validate_issue(self, issue_id: str) -> IssueValidationResult:
 """Validate internal issue from PostgreSQL database"""
 try:
 await self._ensure_database()
 issue_data = await self._db.get_internal_issue(issue_id.upper())
 ...
 # Map database status to IssueStatus enum
 status_map = {
 ...
 # Map database issue_type to IssueType enum
 type_map = {
 "bug": IssueType.BUG,
 ...
 issue_status = status_map.get(issue_data["status"], IssueStatus.OPEN)
 issue_type = type_map.get(issue_data["issue_type"], IssueType.TASK)
 ...
 logger.error(f"Error validating internal issue {issue_id}: {e}")
 return IssueValidationResult(
 valid = False, error_message = f"Failed to validate issue: {str(e)}"
 )

 async def get_issue_metadata(self, issue_id: str) -> Dict[str, Any]:
 """Get internal issue metadata from PostgreSQL database"""
 try:
 await self._ensure_database()
 issue_data = await self._db.get_internal_issue(issue_id.upper())
 ...
 "resolved_at": issue_data["resolved_at"].isoformat()
 if issue_data["resolved_at"]
 else None,
 }
 except Exception as e:
 logger.error(f"Error getting internal issue metadata: {e}")
 ...
 logger.error(f"Error checking user assignment: {e}")
 return False
...
class IssueTrackingService:
 ...
 return (
 False,
 "Emergency override justification must be at least 20 characters",
 )
 ...
 if requirements.enforce_for_production and branch_name in [
 "main",
 "master",
 "production",
 ]:
 ...
 validation_result={
 issue.issue_id: {"valid": result.valid, "error": result.error_message}
 ...
 self, branch_name: str, change_type: str, resource_name: Optional[str] = None
 ) -> List[IssueReference]:
