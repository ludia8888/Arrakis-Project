...
from models.exceptions import ConflictError, ResourceNotFoundError, ValidationError
...
class PropertyService(PropertyServiceProtocol):
 ...
 self, branch: str, property_data: PropertyCreate, created_by: str
 ) -> Property:
 ...
 f"Invalid property: {', '.join(validation_result['errors'])}"
 )
 ...
 branch = branch, document = property_doc
 )
 ...
 async def get_property(self, branch: str, property_id: str) -> Optional[Property]:
 """Get a property by ID"""
 try:
 result = await self.terminus_client.get_document(
 branch = branch, document_id = property_id
 )
 ...
 try:
 # Build query
 query = {"@type": "Property", "branch": branch}

 if object_type:
 query["object_type"] = object_type

 # Execute query using TerminusDB v11.1.14 JSON-based WOQL syntax
 woql_query = {
 "type": "and",
 "clauses": [
 {
 "type": "triple",
 "subject": {"@type": "Variable", "variable_name": "Property"},
 "predicate": {"@type": "Node", "node": "rdf:type"},
 "object": {"@type": "Node", "node": "Property"},
 },
 {
 "type": "triple",
 "subject": {"@type": "Variable", "variable_name": "Property"},
 "predicate": {"@type": "Node", "node": "branch"},
 "object": {"@type": "Value", "value": branch},
 },
 ],
 }
 ...
 branch = branch, document = update_doc
 )
 ...
 self, branch: str, property_id: str, deleted_by: str
 ) -> bool:
 """Delete a property"""
 try:
 ...
 async def validate_property(self, property_data: Dict[str, Any]) -> Dict[str, Any]:
 ...
 type_name = (
 property_data["data_type_id"].split("/")[-1]
 if "/" in property_data["data_type_id"]
 else property_data["data_type_id"]
 )
 if type_name not in self.valid_types:
 errors.append(
 f"Invalid data type. Must be one of: {', '.join(self.valid_types)}"
 )
 ...
 self, branch: str, object_type_id: str
 ) -> List[Property]:
 ...
 logger.error(
 f"Failed to get properties for object type {object_type_id}: {str(e)}"
 )
 raise
