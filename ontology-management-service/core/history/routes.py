...
# Real implementation: using actual event publisher
...
from .models import ChangeOperation, ResourceType, RevertRequest, RevertResult
...
def get_history_event_publisher() -> HistoryEventPublisher:
 ...
 # Real implementation: using DI container
 ...
 try:
 # Use DI container for proper dependency injection
 container = Container()

 # Get properly configured clients from container
 terminus_client = container.terminus_client()
 event_publisher = container.event_publisher()

 return HistoryEventPublisher(
 terminus_client = terminus_client, event_publisher = event_publisher
 )
 ...
 logger = get_logger(__name__)
 logger.warning(f"DI container failed, using fallback: {e}")

 return HistoryEventPublisher(
 terminus_client = get_terminus_client(), event_publisher = get_event_publisher()
 )
...
async def revert_schema(
 ...
 branch = branch, request = request, user_context = user_context
 )
