...
from arrakis_common.utils.security import decrypt_data, encrypt_data, hash_data
...
class PIIHandler:
 ...
 def handle_pii(
 self, data: Dict[str, Any], strategy: Optional[PIIHandlingStrategy] = None
 ) -> Dict[str, Any]:
 """
 PII 처리
 ...
 def anonymize_pii(
 self, data: Dict[str, Any], pii_matches: List[PIIMatch]
 ) -> Dict[str, Any]:
 """PII 익명화"""
 ...
 "password" in field_lower or "passwd" in field_lower or "pwd" in field_lower
 ):
...
def create_pii_handler(environment: str = "development") -> PIIHandler:
 ...
 return PIIHandler(
 encryption_key = encryption_key, strategy = PIIHandlingStrategy.ENCRYPT
 )
