...
class ResilientVersionService:
 ...
 self.timeout = (
 timeout if timeout is not None else config.service.resilience_timeout
 )
 ...
 if time.time() < expiry:
 version_service_cache_hits.labels(
 method = "get", cache_type = "memory"
 ).inc()
 return value
 else:
 ...
 method = func.__name__, error_type = "timeout"
 ).inc()

 except Exception as e:
 last_exception = e
 version_service_errors.labels(
 method = func.__name__, error_type = type(e).__name__
 ).inc()
 ...
 cache_key, result.model_dump() if hasattr(result, "dict") else result
 )
 ...
 self, branch: str, validation: CacheValidation
 ) -> CacheValidation:
 ...
 self._version_service.get_branch_version_summary("main"), timeout = 2.0
 )
 ...
 "last_failure": str(self.circuit_breaker.last_failure)
...
async def get_resilient_version_service(
 redis_client: Redis, config: Optional[Dict[str, Any]] = None
