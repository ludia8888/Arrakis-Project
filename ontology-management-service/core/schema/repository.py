...
class SchemaRepository:
 ...
 self.db_name = os.getenv("TERMINUSDB_DB", "oms") # Use environment variable
 ...
 # Using TerminusDB v11.1.14 JSON-based WOQL syntax
 woql_query = {
 "type": "triple",
 "subject": {"@type": "Variable", "variable_name": "Doc"},
 "predicate": {"@type": "Node", "node": "rdf:type"},
 "object": {"@type": "Node", "node": "SchemaDefinition"},
 }

 if hasattr(self.db, "terminus_client") and self.db.terminus_client:
 query_result = await self.db.terminus_client.query_branch(
 db_name = self.db_name, branch_name = branch, query = woql_query
 )
 ...
 async def create_new_object_type(
 self, branch: str, data: ObjectTypeCreate, author: str
 ) -> bool:
 """
 ...
 if result:
 logger.info(
 f"Schema definition for '{data.name}' successfully created in database"
 )
 return True
 else:
 ...
 logger.error(
 f"Error getting object type by name '{name}': {e}", exc_info = True
 )
 return None

 async def update_object_type(
 self, schema_id: str, branch: str, schema_def: Dict[str, Any], updated_by: str
 ) -> bool:
 """
 Update ObjectType
 """
 ...
 **schema_def,
 ...
 logger.error(
 f"Error updating object type '{schema_id}': {e}", exc_info = True
 )
 return False

 async def get_object_type_by_id(
 self, schema_id: str, branch: str
 ) -> Optional[Dict[str, Any]]:
 """
 ...
 async def mark_object_type_deleted(
 self, schema_id: str, branch: str, deleted_by: str
 ) -> bool:
 """
 ...
 **existing,
 ...
 logger.error(
 f"Error marking object type '{schema_id}' as deleted: {e}",
 exc_info = True,
 )
 return False
