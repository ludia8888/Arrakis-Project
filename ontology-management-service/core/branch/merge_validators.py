...
class StateTransitionValidator(MergeValidator):
 ...
 """스키마에서 상태 전이 규칙 추출"""
 if not self.schema:
 # Fallback to default rules if no schema provided
 return {
 "published": {
 "from": ["review", "draft"],
 ...
 # Extract transition rules from schema definition
 rules = {}
 state_definitions = self.schema.get("stateTransitions", {})

 for state_name, state_config in state_definitions.items():
 if isinstance(state_config, dict):
 rules[state_name] = {
 "from": state_config.get("allowedTransitionsFrom", []),
 "requiredFields": state_config.get("requiredFields", []),
 "validationRules": state_config.get("validationRules", []),
 "conditionalRules": state_config.get("conditionalRules", {}),
 }

 # Merge with default rules for common states
 default_rules = {
 "draft": {
 "from": ["new", "review", "published"],
 "requiredFields": ["title", "content"],
 },
 "review": {
 "from": ["draft"],
 "requiredFields": ["title", "content", "submitted_by", "submitted_at"],
 },
 "published": {
 "from": ["review", "draft"],
 "requiredFields": ["reviewed_by", "published_at", "title", "content"],
 },
 "archived": {
 ...
 "rejected": {
 "from": ["review"],
 "requiredFields": ["reviewed_by", "rejection_reason", "rejected_at"],
 },
 }

 # Merge schema rules with defaults (schema rules take precedence)
 for state, default_rule in default_rules.items():
 if state not in rules:
 rules[state] = default_rule
 else:
 # Merge required fields if not specified
 if not rules[state].get("requiredFields"):
 rules[state]["requiredFields"] = default_rule.get(
 "requiredFields", []
 )
...
class MergeValidatorRegistry:
 ...
 merged_data, base_data, source_data, target_data
 )
