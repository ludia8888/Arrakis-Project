...
class BranchService:
    ...
    async def _load_proposal_from_db(self, proposal_id: str) -> Optional[ChangeProposal]:
        """Load proposal from TerminusDB storage"""
        try:
            async with TerminusDBClient(self.tdb_endpoint) as tdb:
                # Query for the proposal document
                query = f"""
                WOQL.and(
                    WOQL.triple("v:Proposal", "type", "Proposal"),
                    WOQL.triple("v:Proposal", "proposal_id", "{proposal_id}"),
                    WOQL.triple("v:Proposal", "title", "v:Title"),
                    WOQL.triple("v:Proposal", "status", "v:Status"),
                    WOQL.triple("v:Proposal", "source_branch", "v:SourceBranch"),
                    WOQL.triple("v:Proposal", "target_branch", "v:TargetBranch"),
                    WOQL.triple("v:Proposal", "author", "v:Author"),
                    WOQL.triple("v:Proposal", "created_at", "v:CreatedAt")
                )
                """
    ...
    async def list_proposals(
        ...
        try:
            logger.info(f"Listing proposals: branch={branch}, status={status}, author={author}")
            ...
            for proposal in proposals:
                ...
                if branch and branch not in [proposal.source_branch, proposal.target_branch]:
                    continue
    ...
    async def _load_all_proposals_from_db(self) -> List[ChangeProposal]:
        ...
        try:
            async with TerminusDBClient(self.tdb_endpoint) as tdb:
                # Query for all proposal documents
                query = """
                WOQL.and(
                    WOQL.triple("v:Proposal", "type", "Proposal"),
                    WOQL.triple("v:Proposal", "proposal_id", "v:ProposalId"),
                    WOQL.triple("v:Proposal", "title", "v:Title"),
                    WOQL.triple("v:Proposal", "status", "v:Status"),
                    WOQL.triple("v:Proposal", "source_branch", "v:SourceBranch"),
                    WOQL.triple("v:Proposal", "target_branch", "v:TargetBranch"),
    ...
    async def update_proposal(self, proposal_id: str, update: ProposalUpdate, user_id: str) -> ChangeProposal:
        ...
        try:
            ...
            if proposal.author != user_id and not await self._user_can_modify_proposal(user_id, proposal):
                raise PermissionError(f"User {user_id} cannot update proposal {proposal_id}")
            ...
            # Publish event
            if self.event_publisher:
                await self.event_publisher.publish_event("proposal_updated", {
    ...
    async def _user_can_modify_proposal(self, user_id: str, proposal: ChangeProposal) -> bool:
        """Check if user has permission to modify the proposal"""
        # Simple permission check - could be extended with role-based access
        return (proposal.author == user_id or 
                user_id == "admin" or 
                await self._user_is_maintainer(user_id, proposal.target_branch))
        ...
        try:
            async with TerminusDBClient(self.tdb_endpoint) as tdb:
                ...
                doc = {
                    ...
                    "updated_at": proposal.updated_at.isoformat() if proposal.updated_at else datetime.now().isoformat()
                }
                
                await tdb.insert_document(doc, commit_msg=f"Update proposal {proposal.id}")
    ...
    async def approve_proposal(self, proposal_id: str, user_id: str, comment: Optional[str] = None) -> ChangeProposal:
        ...
        try:
            ...
            if not await self._user_can_approve_proposal(user_id, proposal):
                raise PermissionError(f"User {user_id} cannot approve proposal {proposal_id}")
            
            # Check proposal state
            if proposal.status not in [ProposalStatus.OPEN, ProposalStatus.READY_FOR_REVIEW]:
                raise ValueError(f"Proposal {proposal_id} cannot be approved in status {proposal.status}")
            ...
            if self.event_publisher:
                await self.event_publisher.publish_event("proposal_approved", {
                    "proposal_id": proposal_id,
                    "approved_by": user_id,
                    "comment": comment,
                    "source_branch": proposal.source_branch,
            ...
            if await self._should_auto_merge(proposal):
                ...
                try:
                    ...
                    logger.error(f"Auto-merge failed for proposal {proposal_id}: {merge_error}")
    ...
    async def _user_can_approve_proposal(self, user_id: str, proposal: ChangeProposal) -> bool:
    ...
    async def reject_proposal(self, proposal_id: str, user_id: str, reason: str) -> ChangeProposal:
        ...
        try:
            ...
            if not await self._user_can_approve_proposal(user_id, proposal):
                raise PermissionError(f"User {user_id} cannot reject proposal {proposal_id}")
            
            # Check proposal state
            if proposal.status in [ProposalStatus.REJECTED, ProposalStatus.MERGED]:
                raise ValueError(f"Proposal {proposal_id} cannot be rejected in status {proposal.status}")
            ...
            if self.event_publisher:
                await self.event_publisher.publish_event("proposal_rejected", {
                    "proposal_id": proposal_id,
                    "rejected_by": user_id,
                    "reason": reason,
                    "source_branch": proposal.source_branch,
    ...
    async def _notify_proposal_author(self, proposal: ChangeProposal, reviewer_id: str, action: str, message: str):
        ...
        try:
            ...
            logger.info(f"Notification sent to {proposal.author}: Proposal {proposal.id} {action}")
