...
class DiffEngine:
 ...
 logger.warning(
 "DiffEngine: get_branch_info not implemented in TerminusDBClient, using mock data"
 )
 ...
 source_schema, target_schema, format
 )
 ...
 self, base: str, source: str, target: str
 ) -> ThreeWayDiff:
 """3-way diff 계산"""
 # 각 커밋의 스키마 조회
 ...
 base_schema, source_schema, "detailed"
 )

 # Base -> Target 변경사항
 base_to_target = await self._calculate_diff_entries(
 base_schema, target_schema, "detailed"
 )

 # 충돌 감지
 conflicts = await self._detect_conflicts(
 base_to_source, base_to_target, base_schema, source_schema, target_schema
 )
 ...
 async with TerminusDBClient(self.tdb_endpoint) as tdb:
 try:
 # Query for all ObjectTypes in the branch
 object_types_query = {
 "@type": "woql:Select",
 "variables": ["ObjectType", "Name", "Properties"],
 "query": {
 "@type": "woql:And",
 "and": [
 {
 ...
 "object": {"@type": "woql:Node", "node": "sys:Class"},
 },
 {
 "@type": "woql:Triple",
 "subject": {
 ...
 object_types_result = await tdb.query(branch, object_types_query)

 # Process ObjectTypes
 for binding in object_types_result.get("bindings", []):
 object_type_name = binding.get("Name", "")
 if object_type_name and not object_type_name.startswith("sys:"):
 schema["objectTypes"][object_type_name] = {
 "id": binding.get("ObjectType", ""),
 ...
 link_types_query = {
 "@type": "woql:Select",
 "variables": ["LinkType", "Name", "Domain", "Range"],
 "query": {
 "@type": "woql:And",
 "and": [
 {
 ...
 link_types_result = await tdb.query(branch, link_types_query)

 # Process LinkTypes
 for binding in link_types_result.get("bindings", []):
 link_type_name = binding.get("Name", "")
 if link_type_name and not link_type_name.startswith("sys:"):
 schema["linkTypes"][link_type_name] = {
 "id": binding.get("LinkType", ""),
 "name": link_type_name,
 ...
 logger.error(f"Failed to query schema from TerminusDB: {e}")
 ...
 async with TerminusDBClient(self.tdb_endpoint) as tdb:
 try:
 # Query ObjectTypes at specific commit
 object_types_query = {
 "@type": "woql:Select",
 "variables": ["ObjectType", "Name", "Properties"],
 "query": {
 ...
 {
 "@type": "woql:Using",
 "collection": f"CommitData({commit_hash})",
 "query": {
 "@type": "woql:And",
 "and": [
 {
 ...
 object_types_result = await tdb.query(self.db_name, object_types_query)
 ...
 # Query LinkTypes at specific commit
 link_types_query = {
 "@type": "woql:Select",
 "variables": ["LinkType", "Name", "Domain", "Range"],
 "query": {
 ...
 link_types_result = await tdb.query(self.db_name, link_types_query)
 ...
 logger.error(f"Failed to query commit schema from TerminusDB: {e}")
 # Fallback to current main branch schema on error
 logger.warning(
 f"Falling back to main branch schema for commit {commit_hash}"
 )
 ...
 self, source_schema: Dict[str, Any], target_schema: Dict[str, Any], format: str
 ) -> List[DiffEntry]:
 """스키마 간 차이점 계산"""
 entries = []
 ...
 self, source_types: Dict[str, Any], target_types: Dict[str, Any], format: str
 ) -> List[DiffEntry]:
 """ObjectType 차이점 계산"""
 entries = []
 ...
 self, source_links: Dict[str, Any], target_links: Dict[str, Any], format: str
 ) -> List[DiffEntry]:
 """LinkType 차이점 계산"""
 entries = []
 ...
 self, source: Dict[str, Any], target: Dict[str, Any], format: str
 ) -> List[Dict[str, Any]]:
 ...
 changes.append(
 {"field": field, "old": target.get(field), "new": source.get(field)}
 )
 ...
 source_entry, target_entry, base_schema, source_schema, target_schema
 )
 ...
 source_entry.operation == "modify" and target_entry.operation == "delete"
 ) or (
