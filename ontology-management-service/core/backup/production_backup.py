...
# Metrics
backup_chunks = Counter(
 "backup_chunks_total", "Total backup chunks", ["type", "status"]
)
backup_validation = Counter(
 "backup_validation_total", "Backup validation results", ["result"]
)
restore_validation = Counter(
 "restore_validation_total", "Restore validation results", ["result"]
)
backup_encryption_time = Histogram(
 "backup_encryption_seconds", "Time spent encrypting backups"
)
backup_compression_ratio = Gauge(
 "backup_compression_ratio", "Compression ratio achieved"
)
...
class BackupEncryption:
 ...
 if master_key:
 os.environ["CRYPTO_KEY_BACKUP_MASTER"] = base64.b64encode(
 master_key.encode()
 ).decode()
...
class IncrementalBackupTracker:
 ...
 self, component: str, entity_type: str, entity_id: str, operation: str
 ):
 ...
 while current >= since:
 change_key = (
 f"{self.change_log_prefix}{component}:{current.strftime('%Y%m%d')}"
 )
...
class ProductionBackupOrchestrator:
 ...
 self, backup_id: str, chunk_id: str, chunk_data: bytes, key_id: str
 ):
 ...
 async def perform_terminusdb_backup(
 self, backup_type: str = "full", parent_backup_id: Optional[str] = None
 ) -> BackupMetadata:
 ...
 self, component: str, backup_type: str, timestamp: datetime
 ) -> str:
 ...
 return await self._collect_full_backup_data(self.http_client, start_time)
 ...
 client, change["entity_type"], change["entity_id"]
 )
 if doc:
 backup_data["documents"][change["entity_id"]] = doc

 async def _collect_full_backup_data(self, client, start_time: datetime) -> Dict:
 ...
 # Upload in chunks
 chunk_ids = await self._upload_backup_chunks(backup_id, compressed_data, key_id)
 ...
 encrypted_chunk, metadata.encryption_key_id
 )
 ...
 self, backup_id: str, target_branch: Optional[str] = None
 ) -> Dict:
 ...
 logger.info(
 f"Restoring parent backup first: {metadata.parent_backup_id}"
 )
 ...
 backup_id, chunk_id, metadata.encryption_key_id
 )
 ...
 results = await self.apply_incremental_restore(
 backup_dict, target_branch
 )
 else:
 ...
 f"backup:index:{component}", 0, limit - 1
 )
 ...
 backup_id.decode() if isinstance(backup_id, bytes) else backup_id
 )
 ...
 f"backup:metadata:{backup.backup_id}"
 )
 await self.redis_client.zrem(
 f"backup:index:{component}", backup.backup_id
 )
 ...
 time_since_backup = (
 current_time - latest_backup.timestamp
 ).total_seconds() / 60
 ...
 f"/api/document/admin/oms/{entity_type}/{entity_id}"
 )
