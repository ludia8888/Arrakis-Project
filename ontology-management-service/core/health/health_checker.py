...
# Production system monitoring - required for health checks
import psutil
...
class HealthChecker:
 ...
 return HealthCheck(
 "database", False, "Database connection timeout (>5s)", 5000.0
 )
 ...
 self.redis_url, socket_connect_timeout = 5, socket_timeout = 5
 )
 ...
 return HealthCheck("redis", False, "Redis connection timeout (>5s)", 5000.0)
 except Exception as e:
 return HealthCheck(
 "redis",
 False,
 ...
 * (statvfs.f_blocks - statvfs.f_available),
 ...
 if status
 else f"Low disk space: {free_percent:.1f}% free",
 metadata={
 ...
 return HealthCheck(
 "disk_space", False, f"Failed to check disk space: {str(e)}"
 )
 ...
 "MemAvailable", meminfo.get("MemFree", 500000000)
 ),
 "percent": 0,
 },
 )()
 ...
 "Memory",
 (),
 {"total": 1000000000, "available": 500000000, "percent": 50},
 )()
 ...
 if status
 else f"High memory usage: {memory.percent:.1f}%",
 metadata={
 ...
 return HealthCheck("memory", False, f"Failed to check memory: {str(e)}")
 ...
 "cpu",
 status,
 f"{cpu_percent:.1f}% used"
 if status
 else f"High CPU usage: {cpu_percent:.1f}%",
 metadata={
 ...
 return HealthCheck("cpu", False, f"Failed to check CPU: {str(e)}")
 ...
 "unknown", False, f"Unexpected error: {str(result)}"
 )
 ...
 healthy_count = sum(
 1
 for _, check in self.check_history
