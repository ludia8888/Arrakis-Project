...
class IAMIntegration:
 ...
 base_url = self.iam_base_url, verify_ssl = verify_ssl, enable_fallback = True
 )
 ...
 os.getenv("IAM_PERMISSION_CACHE_TTL", "300")
 ) # 5 minutes default
 ...
 cache_key, self._permission_cache_ttl, json.dumps(permissions)
 )
 logger.debug(
 f"Cached permissions for user {user_id} with TTL {self._permission_cache_ttl}s"
 )
 except Exception as e:
 ...
 if not key:
 raise ValueError(
 "🚨 SECURITY ERROR: JWT_SECRET environment variable is required for JWT validation. "
 ...
 "preferred_username", payload.get("username", user_id)
 ),
 ...
 def check_any_scope(
 self, user_context: UserContext, required_scopes: List[str]
 ) -> bool:
 """Check if user has any of the required scopes"""
 user_scopes = set(user_context.metadata.get("scopes", []))
 # Convert enum values to strings if needed
 required_scope_values = [
 s.value if hasattr(s, "value") else s for s in required_scopes
 ]
 ...
 response = await self._http_client.get(
 f"/iam/users/{user_id}/permissions", headers = headers
 )
 ...
 logger.info(
 f"Fetched {len(permissions)} permissions for user {user_id}"
 )
 return permissions
 ...
 logger.error(
 f"Unexpected error getting user permissions for {user_id}: {e}"
 )
 raise IAMServiceUnavailableError(
