...
class PolicyIntegrityChecker:
 ...
 def __init__(
 self,
 snapshot_dir: str = "/etc/oms/snapshots",
 ...
 event_data["previous_snapshot"] = PolicySnapshot(
 **event_data["previous_snapshot"]
 )
 if event_data.get("current_snapshot"):
 event_data["current_snapshot"] = PolicySnapshot(
 **event_data["current_snapshot"]
 )
 ...
 self, previous: PolicySnapshot, current: PolicySnapshot
 ) -> List[TamperingEvent]:
 """변경 사항 분석"""
 events = []
 ...
 size_diff_ratio = abs(current.file_size - previous.file_size) / max(
 previous.file_size, 1
 )
 ...
 field = "policy",
 old_value = str(previous_snapshot.snapshot_hash)
 if previous_snapshot
 else "N/A",
 new_value = str(current_snapshot.snapshot_hash)
 if current_snapshot
 else "N/A",
 tampering_type = tampering_type_map.get(
 event_type, TamperingType.DATA_MANIPULATION
 ),
 ...
 if policy_id:
 filtered_events = [
 e for e in filtered_events if e.metadata.get("policy_id") == policy_id
 ]
 ...
 if target_severity:
 filtered_events = [
 e for e in filtered_events if e.severity == target_severity
 ]
 ...
 critical_count = sum(
 1 for e in recent_events if e.severity == EventSeverity.CRITICAL
 )
 warning_count = sum(
 1 for e in recent_events if e.severity == EventSeverity.HIGH
 )
 ...
 "policies": {},
 "summary": {"clean": 0, "warning": 0, "compromised": 0},
 }
 ...
 event_type = "security.tampering", payload = event.to_dict()
 )
 ...
 # 전체 SIEM 비활성화 체크
 if not os.getenv("ENABLE_SIEM_INTEGRATION", "true").lower() in (
 "true",
 "1",
 "yes",
 ):
 ...
 if not os.getenv("SIEM_SEND_TAMPERING_EVENTS", "true").lower() in (
 "true",
 "1",
 "yes",
 ):
 ...
 # INFO 이벤트는 설정에 따라
 if event.severity == EventSeverity.INFO and os.getenv(
 "SIEM_SEND_INFO_TAMPERING", "false"
 ).lower() in ("true", "1", "yes"):
 return True
...
def get_integrity_checker(
 snapshot_dir: Optional[str] = None, siem_port: Optional[ISiemPort] = None
 ...
 snapshot_dir = snapshot_dir or "/etc/oms/snapshots", siem_port = siem_port
 )
