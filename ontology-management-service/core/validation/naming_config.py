...
# Production JSON serialization - required for performance
import orjson
...
def safe_enum_parse(enum_class: type, value: Any, default: Optional[Any] = None) -> Any:
 ...
 if default is not None:
 logger.warning(
 f"Failed to convert '{value}' to {enum_class.__name__}, using default: {default}"
 )
 return default

 valid_values = [item.value for item in enum_class]
 raise ValueError(
 f"Cannot convert '{value}' to {enum_class.__name__}. Valid values: {valid_values}"
 )
...
class NamingConfigService:
 ...
 def __init__(
 self, config_path: Optional[str] = None, history_path: Optional[str] = None
 ):
 """
 초기화
 ...
 self, convention: NamingConvention, user_id: str, reason: Optional[str] = None
 ) -> NamingConvention:
 """새 명명 규칙 생성"""
 ...
 if hasattr(convention, key) and key not in [
 "id",
 "created_at",
 "created_by",
 ]:
 ...
 self, convention_id: str, user_id: str, reason: Optional[str] = None
 ) -> None:
 """명명 규칙 삭제"""
 if convention_id == "default":
 ...
 return self.create_convention(
 convention,
 user_id,
 reason or f"Imported from {data.get('name', 'external source')}",
 )
 ...
 "needs_migration": compatibility
 == VersionCompatibility.MIGRATION_REQUIRED,
 }
 except Exception as e:
 ...
 self, from_version: str = None, to_version: str = None
 ) -> List[str]:
 """
 버전 간 변경 사항 조회
 ...
 entity_type = parse_enum_with_backward_compatibility(
 entity_type_str, EntityType
 )
 ...
 rule_data_copy[
 "entity_type"
 ] = parse_enum_with_backward_compatibility(
 ...
 logger.error(
 f"Failed to parse rule for entity type '{entity_type_str}': {e}"
 )
 ...
 self, convention_id: str, version: Optional[int] = None
 ) -> List[Dict]:
 """명명 규칙의 변경 이력 조회"""
 ...
 logger.info(
 f"Saved {len(data['conventions'])} conventions to {config_file}"
 )
 except Exception as e:
