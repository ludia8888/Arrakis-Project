...
# Production JSON schema validation - required for validation
import jsonschema
...
class NamingConventionSchema(BaseModel):
 ...
 "1.0.0", description = "Schema version for compatibility", alias = "__version__"
 )
 ...
 name: str = Field(
 ..., min_length = 1, max_length = 200, description = "Human-readable convention name"
 )
 description: Optional[str] = Field(
 None, max_length = 1000, description = "Optional description of the convention"
 )
 rules: Dict[EntityType, NamingRule] = Field(
 ..., description = "Entity type to naming rule mapping"
 )
 reserved_words: List[str] = Field(
 default_factory = list, description = "Words that cannot be used in entity names"
 )
 case_sensitive: bool = Field(
 True, description = "Whether naming validation is case sensitive"
 )
 auto_fix_enabled: bool = Field(
 True, description = "Whether automatic name fixing is enabled"
 )
 ...
 ..., min_length = 1, max_length = 100, description = "User who created the convention"
 )
 ...
 raise ValueError(
 f"Invalid timestamp format: {v}. Expected ISO format with timezone."
 )
 ...
 json_encoders={Enum: enum_encoder, datetime: utc_datetime_encoder},
 json_schema_extra={
...
class NamingRuleSchema(BaseModel):
 ...
 default_factory = list, description = "Required prefixes (at least one must match)"
 )
 required_suffix: Optional[List[str]] = Field(
 default_factory = list, description = "Required suffixes (at least one must match)"
 )
 forbidden_prefix: Optional[List[str]] = Field(
 default_factory = list, description = "Forbidden prefixes"
 )
 forbidden_suffix: Optional[List[str]] = Field(
 default_factory = list, description = "Forbidden suffixes"
 )
 forbidden_words: Optional[List[str]] = Field(
 default_factory = list, description = "Words that cannot appear in the name"
 )
 ...
 None, max_length = 500, description = "Custom regex pattern for validation"
 )
 description: Optional[str] = Field(
 None, max_length = 500, description = "Human-readable description of this rule"
 )
 ...
 if v < min_length:
 raise ValueError(
 f"max_length ({v}) must be >= min_length ({min_length})"
 )
 return v
...
class JsonSchemaValidator:
 ...
 self.schemas[
 "naming_convention"
 ] = NamingConventionSchema.model_json_schema()
 ...
 raise SchemaValidationError(
 f"Naming rule validation failed: {len(errors)} errors found", errors
 )
 except Exception as e:
 raise SchemaValidationError(f"Unexpected validation error: {e}")

 def validate_external_json(
 self, json_data: Union[str, Dict], schema_type: str = "naming_convention"
 ) -> Dict:
 """
 외부 JSON 입력 검증
 ...
 def export_schema(
 self, schema_type: str = "naming_convention", file_path: Optional[str] = None
 ) -> str:
 """
 JSON 스키마를 파일로 내보내기
