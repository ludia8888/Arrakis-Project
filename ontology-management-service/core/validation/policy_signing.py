...
class PolicySigner:
 ...
 def sign_policy(
 self, policy: NamingConvention, algorithm: SignatureAlgorithm, signer: str
 ) -> SignedNamingPolicy:
 """
 정책 서명
 ...
 policy = policy, signature = signature, integrity_hash = policy_hash
 )
 ...
 if is_valid:
 logger.info(
 f"Policy signature verified for signer: {signed_policy.signature.signer}"
 )
 else:
 ...
 self, data: bytes, signature: str, algorithm: SignatureAlgorithm
 ) -> bool:
 """서명 검증"""
 try:
 if algorithm == SignatureAlgorithm.HMAC_SHA256:
...
class PolicySigningManager:
 ...
 if not algorithm:
 algorithm = SignatureAlgorithm(
 self.config.get("default_algorithm", "HMAC-SHA256")
 )
...
def get_policy_signing_manager(
 config_path: Optional[str] = None,
) -> PolicySigningManager:
 """정책 서명 관리자 인스턴스 반환"""
