...
from utils.retry_strategy import calculate_delay

from .models import DLQMessage, MessageStatus, RetryConfig, RetryStrategy
...
class DLQHandler:
 ...
 def __init__(
 self, message_store: MessageStore, default_config: Optional[RetryConfig] = None
 ):
 ...
 message.next_retry_at = self._calculate_next_retry(message.retry_count)

 # Store message
 success = await self.store.store(message)

 if success:
 self.logger.info(f"Message {message.id} sent to DLQ {queue_name}")
 else:
 self.logger.error(f"Failed to store message {message.id} in DLQ")

 return message

 async def retry_message(
 self, queue_name: str, message_id: str, retry_handler: callable
 ) -> bool:
 """Retry a DLQ message"""
 # Get message
 ...
 self.logger.warning(f"Cannot retry poison message {message_id}")
 return False
 ...
 message.original_queue, message.content, message.metadata
 )
 ...
 self.logger.info(f"Successfully retried message {message_id}")
 return True
 else:
 ...
 await self._handle_retry_failure(message, f"Retry exception: {str(e)}")
 return False

 async def process_retry_batch(
 self, queue_name: str, retry_handler: callable, batch_size: Optional[int] = None
 ) -> Dict[str, int]:
 """Process a batch of messages ready for retry"""
 batch_size = batch_size or self.config.batch_size

 # Get messages ready for retry
 messages = await self.store.get_ready_for_retry(queue_name, limit = batch_size)
 ...
 tasks = [
 self.retry_message(queue_name, msg.id, retry_handler) for msg in messages
 ]
 ...
 return {"processed": len(messages), "succeeded": succeeded, "failed": failed}

 async def get_message_status(
 self, queue_name: str, message_id: str
 ...
 async def requeue_message(self, queue_name: str, message_id: str) -> bool:
 ...
 self, queue_name: str, message_id: str, reason: Optional[str] = None
 ) -> bool:
 ...
 async def cleanup_expired(self, queue_name: str, limit: int = 100) -> int:
 """Clean up expired messages"""
 expired_messages = await self.store.get_expired(queue_name, limit)
 ...
 if deleted_count > 0:
 self.logger.info(f"Cleaned up {deleted_count} expired messages")
 ...
 * (self.config.backoff_multiplier**retry_count),
 ...
 async def _handle_retry_failure(self, message: DLQMessage, error: str):
 ...
 message.mark_as_poison()
 message.add_error(f"Marked as poison after {message.retry_count} retries")

 # Move to poison queue if configured
 poison_key = RedisKeyPatterns.DLQ_POISON.format(queue_name = message.queue_name)

 await self.store.update(message)

 self.logger.error(f"Message {message.id} marked as poison")
