...
class RateLimitCoordinator:
 ...
 self._strategies[RateLimitAlgorithm.SLIDING_WINDOW] = SlidingWindowStrategy(
 self.default_config
 )
 self._strategies[RateLimitAlgorithm.TOKEN_BUCKET] = TokenBucketStrategy(
 self.default_config
 )
 self._strategies[RateLimitAlgorithm.LEAKY_BUCKET] = LeakyBucketStrategy(
 self.default_config
 )
 ...
 self, key: RateLimitKey, config: RateLimitConfig
 ) -> RateLimitResult:
 ...
 user_id = key.identifier, endpoint = key.endpoint or "global"
 )
 elif key.scope == RateLimitScope.IP:
 return RedisKeyPatterns.RATE_LIMIT_IP.format(
 ip = key.identifier, endpoint = key.endpoint or "global"
 )
 else:
