...
from .models import RateLimitAlgorithm, RateLimitConfig
...
class AdaptiveRateLimiter:
 ...
 def adjust_limits(
 self, config: RateLimitConfig, load_factor: float
 ) -> RateLimitConfig:
 """
 ...
 adjusted_limit = max(
 config.min_requests, min(config.max_requests, adjusted_limit)
 )
 ...
 self, load_factor: float, config: RateLimitConfig
 ) -> float:
 """
 ...
 return {"status": "no_data", "message": "No load history available"}
 ...
 variance = sum((x - avg_load) ** 2 for x in self._load_history) / len(
 self._load_history
 )
