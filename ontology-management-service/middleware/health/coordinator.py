...
class HealthCoordinator:
 ...
 check_tasks = [check.execute() for check in self._health_checks.values()]
 ...
 if health_data:
 dependency_status[dep] = HealthStatus(
 ...
 self._failure_counts[result.name] = (
 self._failure_counts.get(result.name, 0) + 1
 )
 ...
 # Real implementation: Send alert to notification system and audit-service
 try:
 import os

 import httpx
 from core.notification.alert_dispatcher import AlertDispatcher

 # Production audit integration - direct HTTP call to audit-service
 audit_service_url = os.getenv(
 "AUDIT_SERVICE_URL", "http://audit-service:8001"
 )
 audit_payload = {
 "event_type": "health_alert_created",
 ...
 # Send audit event to audit-service asynchronously
 try:
 async with httpx.AsyncClient(timeout = 3.0) as client:
 await client.post(
 f"{audit_service_url}/api/v2/events/direct",
 json = audit_payload,
 )
 except Exception as audit_error:
 logger.warning(f"Failed to send audit event: {audit_error}")

 # Send alert notification
 alert_dispatcher = AlertDispatcher()
 await alert_dispatcher.dispatch_health_alert(
 alert = alert,
 ...
 logger.info(f"Alert notification sent for {alert_key}")

 except Exception as e:
 logger.error(f"Failed to send alert notification for {alert_key}: {e}")
 # Continue despite notification failure to ensure health monitoring continues
 ...
 f"health_check_{result.status.value}_total", labels={"check": result.name}
 )
 ...
 key, health.to_dict(), expire = timedelta(seconds = self.check_interval * 2)
 )
