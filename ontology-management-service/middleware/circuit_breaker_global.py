...
class GlobalCircuitBreaker:
 ...
 def __init__(
 self, config: GlobalCircuitConfig, redis_client: Optional[redis.Redis] = None
 ):
 self.config = config
 ...
 # 윈도우에 성공 기록 (0)
 if (
 len(self.metrics.error_rate_window)
 ...
 or (
 len(self.metrics.error_rate_window) >= 10
 and self.metrics.recent_error_rate
 ...
 data.get("updated_at", current_time.isoformat())
 )
 ...
 "metrics_summary": {
 "total_requests": instance_data.get(
 "metrics", {}
 ).get("total_requests", 0),
 "failed_requests": instance_data.get(
 "metrics", {}
 ...
 logger.info(
 f"Forced distributed synchronization completed for {self.config.service_name}"
 )
 ...
 [i for i in instances if i.get("state") == "closed"]
 )
 degraded_instances = len(
 [i for i in instances if i.get("state") == "half_open"]
 )
