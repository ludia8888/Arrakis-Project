...
app.conf.update(
    ...
    task_queues=(
        ...
        Queue(
            "validation", Exchange("tasks"), routing_key="task.validation.#", priority=5
        ),
...
def task_prerun_handler(
    ...
    """Log task start"""
    logger.info(
        "Task starting", task_id=task_id, task_name=task.name, args=args, kwargs=kwargs
    )
...
def task_retry_handler(sender=None, task_id=None, reason=None, **kw):
    """Log task retry"""
    logger.warning(
        "Task retrying", task_id=task_id, task_name=sender.name, reason=str(reason)
    )
