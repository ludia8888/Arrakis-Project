...
from shared.proto_stubs import scheduler_service_pb2, scheduler_service_pb2_grpc
...
class SchedulerClient:
 ...
 "current_execution": self._execution_proto_to_dict(
 response.current_execution
 )
 if response.HasField("current_execution")
 else None,
 "next_run_time": self._timestamp_to_datetime(response.next_run_time)
 if response.HasField("next_run_time")
 else None,
 }
 ...
 self, job_id: str, limit: int = 50, status_filter: Optional[str] = None
 ) -> List[Dict[str, Any]]:
 ...
 job_id = job_id, limit = limit, status_filter = status_filter
 )
 ...
 result["next_run_time"] = self._timestamp_to_datetime(
 job_proto.next_run_time
 )
 if job_proto.HasField("last_run_time"):
 result["last_run_time"] = self._timestamp_to_datetime(
 job_proto.last_run_time
 )
