...
class TokenValidationRequest(BaseModel):
    ...
    required_scopes: Optional[List[str]] = Field(
        None, description="Required scopes for access"
    )
...
class ServiceAuthRequest(BaseModel):
    ...
    requested_scopes: List[str] = Field(
        default_factory=list, description="Scopes needed"
    )
...
class ScopeCheckResponse(BaseModel):
    ...
    user_scopes: List[str] = Field(
        default_factory=list, description="User's actual scopes"
    )
    missing_scopes: List[str] = Field(
        default_factory=list, description="Missing scopes"
    )
...
SCOPE_HIERARCHY = {
    ...
    "api:ontologies:admin": ["api:ontologies:write", "api:ontologies:read"],
    "api:ontologies:write": ["api:ontologies:read"],
    "api:schemas:write": ["api:schemas:read"],
    "api:branches:write": ["api:branches:read"],
    "api:proposals:write": ["api:proposals:read"],
    "api:proposals:approve": ["api:proposals:read"],
}
