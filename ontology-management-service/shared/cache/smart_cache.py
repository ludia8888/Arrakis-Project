...
def increment_cache_operations(operation: str, tier: str):
 ...
 "cache_operations_total", 1, {"operation": operation, "tier": tier}
 )
...
class CacheMetrics:
 ...
 terminus_hit_ratio = (
 self.terminus_hits / total_terminus if total_terminus > 0 else 0
 )
...
class SmartCache(CachePort):
 ...
 logger.warning(
 "SmartCache: Redis client was not provided during initialization. Distributed cache is disabled."
 )
 ...
 cache_key, terminus_value, self.distributed_ttl
 )
 ...
 if self.terminus_client:
 terminus_success = await self._set_to_terminus(
 cache_key, value, terminus_ttl
 )
 if terminus_success:
 ...
 stats["memory"]["redis_used"] = redis_info.get(
 ...
 observe_cache_latency("get", "redis", duration)
 ...
 duration = asyncio.get_event_loop().time() - start_time
 observe_cache_latency("set", "redis", duration)
 return True
 except Exception as e:
 ...
 duration = asyncio.get_event_loop().time() - start_time
 observe_cache_latency("delete", "redis", duration)
 return True
 except Exception as e:
 ...
 "object": {
 "@type": "Value",
 "data": {"@type": "xsd:string", "@value": "CacheEntry"},
 },
 }
 ...
 db_name = "oms"
 await self.terminus_client.query(
 db_name, woql_query, f"Cache delete: {key}"
 )
