...
# Production import - asyncpg must be available
import asyncpg
...
class PostgresConnector:
 ...
 query, *params if params else [], timeout = timeout
 )
 else:
 async with self._pool.acquire() as conn:
 return await conn.execute(
 ...
 converted_query, converted = self._convert_named_params(
 query, params
 )
 else:
 ...
 converted_query, converted_params, timeout = timeout
 )
 except Exception as e:
 ...
 query, *params if params else [], column = column, timeout = timeout
 )
 ...
 async def create_tables_from_sqlite_schema(
 self, sqlite_schema: List[str]
 ) -> List[str]:
 """
