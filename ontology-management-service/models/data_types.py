...
class DataType(BaseModel):
 ...
 errors.append(
 "Map types must specify both map_key_type and map_value_type"
 )
 ...
 if constraint.constraint_type == "enum" and not isinstance(
 constraint.value, list
 ):
 errors.append("Enum constraint value must be a list")
 elif (
 constraint.constraint_type in ["min", "max"]
 and self.category != DataTypeCategory.PRIMITIVE
 ):
...
SYSTEM_DATA_TYPES = {
 ...
 "metadata": {"default_precision": 18, "default_scale": 2},
 },
 # Temporal types
 "date": {
 "name": "date",
 ...
 "compatible_types": ["string"],
 "metadata": {"format": "HH:MM:SS[.fff]", "example": "14:30:00"},
 },
 "datetime": {
 "name": "datetime",
 ...
 "metadata": {"requires_item_type": True, "supports_nested_arrays": True},
 },
 "map": {
 "name": "map",
 "display_name": "Map",
 ...
 "metadata": {
 "structure": {"lat": "double", "lon": "double"},
 ...
 "metadata": {"element_type": "float", "supports_sparse": True},
 },
 "timeseries": {
 "name": "timeseries",
 ...
 "supported_operations": ["size", "mime_type", "is_null", "is_not_null"],
 "constraints": [
 TypeConstraint(
 ...
 "metadata": {"reference_type": "entity", "supports_foreign_key": True},
 },
}
...
class DataTypeValidator:
 ...
 errors.append(
 f"Value cannot be null for non-nullable type {data_type.name}"
 )
 ...
 errors.append(
 constraint.message
 or f"Constraint {constraint.constraint_type} failed"
 )
 ...
 # Missing required modules for validation
 import logging

 logger = logging.getLogger(__name__)
 logger.warning(
 f"Format validation failed - missing module for {format}: {e}"
 )
 return False
 except Exception as e:
 # Log unexpected format validation errors
 import logging

 logger = logging.getLogger(__name__)
 logger.error(
 f"Unexpected format validation error for {format} with value {value}: {e}",
 exc_info = True,
 )
 return False
 ...
 return (
 len(str(value).replace(".", "").lstrip("-")) <= constraint.value
 )
 return True
 ...
 # Value type incompatible with constraint (e.g., comparing string to number)
 import logging

 logger = logging.getLogger(__name__)
 logger.warning(
 f"Constraint '{constraint.constraint_type}' type mismatch for value {value}: {e}"
 )
 return False
 except AttributeError as e:
 # Missing constraint attributes or value attributes
 import logging

 logger = logging.getLogger(__name__)
 logger.warning(
 f"Constraint '{constraint.constraint_type}' attribute error for value {value}: {e}"
 )
 return False
 except Exception as e:
 # Log unexpected constraint validation errors
 import logging
...
class DataTypeConverter:
 ...
 @staticmethod
 def convert(
 value: Any, from_type: str, to_type: str
 ...
 False,
 None,
 f"Conversion from {from_type} to {to_type} not implemented",
 )
 ...
 **type_def,
 ...
 def convert_value(
 self, value: Any, from_type_id: str, to_type_id: str
 ) -> Tuple[bool, Any, Optional[str]]:
