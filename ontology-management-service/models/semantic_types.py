...
class ValidationRule(BaseModel):
 ...
 False,
 self.error_message
 or f"Value does not match pattern: {self.value}",
 )
 ...
 return (
 False,
 self.error_message or f"Length must be >= {self.value}",
 )
 ...
 False,
 self.error_message
 or f"Value must be one of: {', '.join(map(str, self.value))}",
 )
...
class SemanticType(BaseModel):
 ...
 default_factory = list, description = "List of validation rules to apply"
 )
 ...
 None, description = "Input mask for UI (e.g., '(999) 999-9999' for phone)"
 )
 ...
 # Invalid format string or value incompatible with format
 import logging

 logger = logging.getLogger(__name__)
 logger.warning(
 f"Display format '{self.display_format}' failed for value {value}: {e}"
 )
 return str(value)
 except Exception as e:
 # Log unexpected display formatting errors
 import logging

 logger = logging.getLogger(__name__)
 logger.error(
 f"Unexpected display formatting error for format '{self.display_format}' and value {value}: {e}"
 )
 return str(value)
 return str(value)
...
PREDEFINED_SEMANTIC_TYPES = {
 ...
 ValidationRule(
 type = ConstraintType.MAX_LENGTH, value = 2048, error_message = "URL too long"
 ),
 ],
...
class SemanticTypeRegistry:
 ...
 raise ValueError(
 f"Cannot override system semantic type: {semantic_type.id}"
 )
