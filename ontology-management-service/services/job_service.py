...
class JobService:
 ...
 terminus_endpoint = os.environ.get(
 "TERMINUSDB_ENDPOINT", "http://localhost:6363"
 )

 # Security: No default password - fail if not provided
 terminusdb_password = os.environ.get("TERMINUSDB_ADMIN_PASS")
 if not terminusdb_password:
 raise ValueError(
 "🚨 SECURITY ERROR: TERMINUSDB_ADMIN_PASS environment variable is required. "
 "No default password allowed for production security."
 )
 ...
 # Create PostgreSQL client for production
 ...
 logger.info(
 "Job created", job_id = job.id, job_type = job_type, created_by = created_by
 )
 ...
 collection = "jobs", query={"id": job_id}, limit = 1
 )
 ...
 async def list_jobs(
 self, filters: Optional[JobFilter] = None, limit: int = 100, offset: int = 0
 ) -> List[Job]:
 ...
 # Get documents
 docs = await self.db_client.read(
 collection = "jobs", query = query, limit = limit, offset = offset
 )
 ...
 async def get_job_stats(self, filters: Optional[JobFilter] = None) -> JobStats:
 """Get job statistics"""
 ...
 if stats.total > 0:
 stats.retry_rate = (
 sum(1 for j in jobs if j.metadata.retry_count > 0) / stats.total
 )
 ...
 jobs = await self.list_jobs(JobFilter(status = [JobStatus.IN_PROGRESS]))

 for job in jobs:
 if job.started_at and job.started_at < stuck_threshold:
 stuck_jobs.append(job.id)
 logger.warning(
 "Found stuck job", job_id = job.id, started_at = job.started_at
 )
 ...
 collection = "jobs", document = doc, message = f"Create job {job.id}"
 )
 except Exception as e:
 ...
 collection = "jobs", doc_id = job_id, message = f"Delete job {job_id}"
 )
 except Exception as e:
 logger.error(f"Error deleting job {job_id}: {e}")
 raise

 async def _find_job_by_idempotency_key(self, idempotency_key: str) -> Optional[Job]:
 """Find job by idempotency key"""
 await self.initialize()

 try:
 docs = await self.db_client.read(
 collection = "jobs", query={"idempotency_key": idempotency_key}, limit = 1
 )
 ...
 await self._redis_client.publish(f"job:events:{job.id}", json.dumps(event))

 # Publish to global channel
 await self._redis_client.publish("job:events:all", json.dumps(event))
