...
# Production typing support - required for Python 3.10+
...
# Production OpenTelemetry instrumentation - required for tracing
...
class JaegerTracingManager:
 ...
 self.tracer_provider = TracerProvider(resource = self.config.get_resource())
 ...
 logger.info(
 f"Jaeger tracing initialized: {self.config.agent_host}:{self.config.agent_port}"
 )
 ...
 name = name, kind = kind, attributes = self._sanitize_attributes(attributes or {})
 )
 ...
 self, operation_name: str, operation_type: str = "graph_analysis", **attributes
 ):
 ...
 **attributes,
 ...
 "error", operation_name, span, error = str(e)
 )
 ...
 with self.tracer.start_as_current_span(
 name = span_name, attributes = self._sanitize_attributes(attributes)
 ) as span:
 try:
