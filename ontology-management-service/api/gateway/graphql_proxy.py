...
from models.exceptions import ValidationError, ServiceUnavailableError
...
class GraphQLProxy:
    ...
    async def execute(
        self, request: GraphQLRequest, context: Dict[str, Any]
    ) -> GraphQLResponse:
        """GraphQL 요청 실행"""
    ...
    def _get_operation(
        ...
        if not operations:
            raise ValidationError("No operation found in query")

        if operation_name:
            for op in operations:
                if op.name and op.name.value == operation_name:
                    return op
            raise ValidationError(f"Operation '{operation_name}' not found")

        if len(operations) > 1:
            raise ValidationError("Multiple operations found, operation name required")
    ...
    async def _execute_mutation(
        ...
        try:
            ...
            if not selections:
                raise ValidationError("No mutation field found")

            # 첫 번째 mutation 필드 처리 (일반적으로 하나만 있음)
            field = selections[0]
            if not isinstance(field, ast.FieldNode):
                raise ValidationError("Invalid mutation field")

            mutation_name = field.name.value
            arguments = self._extract_arguments(field, request.variables)

            # REST API로 매핑
            result = await self._map_mutation_to_rest(mutation_name, arguments, context)

            # GraphQL 응답 형식으로 변환
            return GraphQLResponse(data={mutation_name: result})

        except Exception as e:
            logger.error(f"Mutation execution error: {e}")
            return GraphQLResponse(
                errors=[{"message": str(e), "extensions": {"code": "MUTATION_ERROR"}}]
            )

    def _extract_arguments(
        self, field: ast.FieldNode, variables: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """필드 인자 추출"""
    ...
    async def _map_mutation_to_rest(
        self, mutation_name: str, arguments: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if not service_url:
            raise ValidationError(f"Service endpoint not configured: {mapping['service']}")
        ...
        response = await self.http_client.request(
            method=mapping["method"], url=url, content=body, headers=headers
        )
        ...
        if response.status_code >= 200 and response.status_code < 300:
            ...
            raise Exception(
                f"REST API call failed: {response.status_code} - {response.text}"
            )
