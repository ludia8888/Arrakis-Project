...
class Subscription:
 ...
 # 특정 리소스 타입만 필터링
 filter_subjects = [
 f"oms.schema.*.{rt.value.lower()}.*" for rt in resource_types
 ]
 else:
 # 모든 스키마 변경 이벤트
 ...
 resource_type = ResourceTypeEnum(
 event_data.get("resource_type", "object_type")
 ),
 ...
 logger.info(
 f"Schema changes subscription cancelled for user {user.user_id}"
 )
 break
 ...
 logger.info(
 f"Cleaned up schema changes subscription for user {user.user_id}"
 )

 @strawberry.subscription
 async def branch_changes(
 self, info: Info, branch_names: Optional[list[str]] = None
 ...
 if branch_names:
 filter_subjects = [
 f"oms.branch.*.{branch}" for branch in branch_names
 ]
 else:
 ...
 logger.info(
 f"Branch changes subscription cancelled for user {user.user_id}"
 )
 break
 ...
 logger.info(
 f"Cleaned up branch changes subscription for user {user.user_id}"
 )
 ...
 logger.info(
 f"Proposal updates subscription cancelled for user {user.user_id}"
 )
 break
 ...
 logger.info(
 f"Cleaned up proposal updates subscription for user {user.user_id}"
 )
 ...
 if job and job.created_by != user.user_id:
 # 다른 사용자의 작업이므로 이벤트 무시
 logger.debug(
 f"Filtering out job {job_id} - not owned by user {user.user_id}"
 )
 return
 except Exception as job_check_error:
 logger.warning(
 f"Failed to check job ownership for {job_id}: {job_check_error}"
 )
 # 에러가 발생한 경우 안전하게 이벤트를 필터링
 return
 else:
 # job_id가 없는 경우 이벤트 무시
 logger.debug(
 "Filtering out event without job_id when user_jobs_only = True"
 )
 return
 ...
 job_id = event_data.get("job_id", ""),
 status = event_data.get("status", ""),
 progress_percentage = event_data.get(
 ...
 logger.info(
 f"Action progress subscription cancelled for user {user.user_id}"
 )
 break
 ...
 logger.info(
 f"Cleaned up action progress subscription for user {user.user_id}"
 )
