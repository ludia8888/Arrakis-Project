...
async def create_semantic_type(
 ...
 """
 logger.info(
 f"Creating semantic type: {semantic_type_data.name} by user: {current_user}"
 )
 ...
 raise HTTPException(
 status_code = 400, detail = f"Semantic type with ID '{type_id}' already exists"
 )
...
async def list_semantic_types(
 category: Optional[SemanticTypeCategory] = Query(
...
async def get_semantic_type(
 semantic_type_id: str, current_user: str = Depends(get_current_user)
) -> SemanticType:
 """
 ...
 raise HTTPException(
 status_code = 404, detail = f"Semantic type '{semantic_type_id}' not found"
 )
...
async def update_semantic_type(
 ...
 raise HTTPException(
 status_code = 403, detail = "Cannot modify system semantic types"
 )
...
async def delete_semantic_type(
 ...
 if semantic_type.is_system:
 raise HTTPException(
 status_code = 403, detail = "Cannot delete system semantic types"
 )
 ...
 # Query properties that reference this semantic type
 properties_using_semantic = (
 await property_repo.find_properties_by_semantic_type(
 semantic_type_id = semantic_type_id
 )
 )

 if properties_using_semantic:
 property_names = [
 prop.get("name", "unknown") for prop in properties_using_semantic
 ]
 raise HTTPException(
 status_code = 409,
 ...
 dependent_structs = []
 for struct_type in struct_type_registry.list_all():
 for field in struct_type.fields:
 if field.semantic_type_id == semantic_type_id:
 dependent_structs.append(struct_type.name)
 break
 ...
 logger.warning(
 f"Could not check dependencies for semantic type {semantic_type_id}: {e}"
 )
 ...
 {"semantic_type_id": semantic_type_id, "deleted_by": current_user},
 )
...
async def list_categories(current_user: str = Depends(get_current_user)) -> List[str]:
 """
...
async def bulk_validate(
 validations: Dict[str, ValidationRequest] = Body(
 ..., description = "Map of semantic_type_id to validation request"
 ),
 current_user: str = Depends(get_current_user),
 ...
 is_valid = False, errors = [f"Unknown semantic type: {semantic_type_id}"]
 )
 continue
