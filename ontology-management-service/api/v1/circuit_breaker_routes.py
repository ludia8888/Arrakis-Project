...
from middleware.circuit_breaker_global import get_global_circuit_breaker, GlobalCircuitState
...
async def get_circuit_breaker_status(
    ...
    if not circuit_breaker:
        raise HTTPException(
            status_code=503,
            detail="Global circuit breaker not initialized"
        )
...
@router.post("/reset", dependencies=[Depends(require_scope([IAMScope.ONTOLOGIES_WRITE]))])
...
def _get_recommendations(status: Dict[str, Any]) -> list:
    ...
    if metrics.get("consecutive_failures", 0) > 0:
        recommendations.append(f"연속 실패: {metrics['consecutive_failures']}회 - 시스템 안정성을 검토하세요.")
...
async def get_distributed_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve distributed status: {str(e)}"
        )

@router.post("/distributed/sync", dependencies=[Depends(require_scope([IAMScope.ONTOLOGIES_WRITE]))])
...
async def get_distributed_health(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve distributed health: {str(e)}"
        )
...
def _get_distributed_recommendations(distributed_health: Dict[str, Any]) -> List[str]:
    ...
    if status == "disabled":
        ...
        recommendations.append("다른 OMS 인스턴스가 감지되지 않습니다. 단일 인스턴스 환경이거나 네트워크 분할이 발생했을 수 있습니다.")
...
def _analyze_cluster_health(distributed_health: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "risk_assessment": _assess_cluster_risk(health_ratio, total_instances, failed_instances)
    }

def _assess_cluster_risk(health_ratio: float, total_instances: int, failed_instances: int) -> str:
    """클러스터 위험도 평가"""
    if total_instances <= 1:
