...
async def get_gateway_mode():
    ...
    return {
        ...
        "data_kernel_endpoint": os.getenv(
            "DATA_KERNEL_GRPC_ENDPOINT", "not configured"
        ),
        "microservices": {
            "embedding_service": os.getenv("USE_EMBEDDING_MS", "false").lower()
            == "true",
            "scheduler_service": os.getenv("USE_SCHEDULER_MS", "false").lower()
            == "true",
...
async def get_microservices_status():
    ...
    return {
        ...
        if os.getenv("USE_DATA_KERNEL_GATEWAY", "false").lower() == "true"
        else "monolith",
        "services": {
            "data_kernel": {
                "enabled": os.getenv("USE_DATA_KERNEL_GATEWAY", "false").lower()
                == "true",
        ...
        "environment": {
            ...
            "iam_validation": os.getenv("USE_IAM_VALIDATION", "false").lower()
            == "true",
...
async def get_migration_progress():
    ...
    return {
        ...
        "current_status": {
            "data_kernel": "migrated"
            if os.getenv("USE_DATA_KERNEL_GATEWAY", "false").lower() == "true"
            else "pending",
            "embedding_service": "migrated"
            if os.getenv("USE_EMBEDDING_MS", "false").lower() == "true"
            else "pending",
            "scheduler_service": "migrated"
            if os.getenv("USE_SCHEDULER_MS", "false").lower() == "true"
            else "pending",
            "event_gateway": "migrated"
            if os.getenv("USE_EVENT_GATEWAY", "false").lower() == "true"
            else "pending",
        },
        "next_steps": [
