...
class BatchObjectTypesRequest(BatchRequest):
 ...
 False, description = "Include properties in response"
 )
...
class BatchPropertiesRequest(BaseModel):
 ...
 object_type_ids: List[str] = Field(
 ..., description = "Object type IDs to load properties for", max_items = 100
 )
...
class BatchLinkTypesRequest(BatchRequest):
 ...
 False, description = "Include source/target object types"
 )
...
@router.post(
 ...
 dependencies = [Depends(require_scope([IAMScope.SCHEMAS_READ]))],
)
async def batch_get_object_types(
 ...
 try:
 logger.info(
 f"Batch loading {len(request.ids)} object types for user {current_user.user_id}"
 )
...
async def batch_get_properties(
 ...
 try:
 logger.info(
 f"Batch loading properties for {len(request.object_type_ids)} object types"
 )
 ...
 if isinstance(result, Exception):
 logger.warning(
 f"Failed to load properties for {object_type_id}: {result}"
 )
 data[object_type_id] = []
 elif result is None:
 data[object_type_id] = []
...
async def batch_get_branches(
 ...
 # Production implementation using actual BranchService
 data = {}

 for branch_id in request.ids:
 try:
 # Get branch info from BranchService
 ...
 "modified_at": branch.modifiedAt.isoformat()
 if branch.modifiedAt
 else None,
 "modified_by": branch.modifiedBy,
 "version_hash": branch.versionHash,
 "is_locked": getattr(branch, "isLocked", False),
 }

 # Include state information if requested
 if request.include_state:
 try:
 # Get additional state information from TerminusDB
 branch_info = await branch_service._get_branch_info(
 branch_id
 )
 if branch_info:
 branch_data["state"] = {
 "commit_hash": branch_info.get("head", "unknown"),
 "last_modified": branch.modifiedAt.isoformat()
 if branch.modifiedAt
 else datetime.utcnow().isoformat(),
 "object_count": branch_info.get("object_count", 0),
 "is_dirty": branch_info.get("is_dirty", False),
 "branch_type": branch_info.get("type", "feature"),
 }
 else:
 # Fallback state if detailed info not available
 branch_data["state"] = {
 "commit_hash": branch.versionHash,
 "last_modified": branch.modifiedAt.isoformat()
 if branch.modifiedAt
 else datetime.utcnow().isoformat(),
 "object_count": 0,
 ...
 logger.warning(
 f"Could not get state for branch {branch_id}: {state_error}"
 )
 # Provide minimal state info
 branch_data["state"] = {
 "commit_hash": branch.versionHash,
 "last_modified": branch.modifiedAt.isoformat()
 if branch.modifiedAt
 ...
 # Branch not found - return minimal info
 ...
 logger.error(f"Error loading branch {branch_id}: {e}")
 # Return error info for this specific branch
 data[branch_id] = {
 "id": branch_id,
 "name": branch_id,
 "error": f"Failed to load branch: {str(e)}",
 "exists": False,
 }
 ...
 "count": len(data),
 "timestamp": datetime.utcnow().isoformat(),
 "success_count": len([b for b in data.values() if not b.get("error")]),
 "error_count": len([b for b in data.values() if b.get("error")]),
 }
...
async def get_batch_metrics(
 req: Request, current_user: UserContext = Depends(get_current_user)
):
 """
