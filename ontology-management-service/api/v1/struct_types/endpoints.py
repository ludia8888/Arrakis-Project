...
from models.struct_types import StructFieldDefinition, StructType, struct_type_registry
...
class StructTypeCreate(BaseModel):
 ...
 ..., description = "List of field definitions", min_items = 1
 )
...
async def create_struct_type(
 struct_type_data: StructTypeCreate, current_user: str = Depends(get_current_user)
) -> StructType:
 """
 ...
 """
 logger.info(
 f"Creating struct type: {struct_type_data.name} by user: {current_user}"
 )
 ...
 raise HTTPException(
 status_code = 400, detail = f"Struct type with ID '{type_id}' already exists"
 )
 ...
 if field_data.data_type_id.startswith("struct:") or struct_type_registry.exists(
 field_data.data_type_id
 ):
 raise HTTPException(
 status_code = 400,
 detail=(
...
async def get_struct_type(
 struct_type_id: str, current_user: str = Depends(get_current_user)
) -> StructType:
 """
 ...
 if not struct_type:
 raise HTTPException(
 status_code = 404, detail = f"Struct type '{struct_type_id}' not found"
 )
...
async def update_struct_type(
 ...
 raise HTTPException(status_code = 403, detail = "Cannot modify system struct types")
 ...
 # Check for nested struct references
 if field_data.data_type_id.startswith(
 "struct:"
 ) or struct_type_registry.exists(field_data.data_type_id):
 raise HTTPException(
 status_code = 400,
 detail=(
 f"Nested structs are not supported. Field '{field_data.name}' "
...
async def delete_struct_type(
 ...
 # Check if system type
 if struct_type.is_system:
 raise HTTPException(status_code = 403, detail = "Cannot delete system struct types")

 # Real implementation: Check if type is in use by any properties
 try:
 from core.repositories.property_repository import PropertyRepository
 from database.clients.terminus_db import get_terminus_client

 property_repo = PropertyRepository(get_terminus_client())

 # Query properties that reference this struct type
 properties_using_struct = await property_repo.find_properties_by_data_type(
 data_type_id = struct_type_id
 )

 if properties_using_struct:
 property_names = [
 prop.get("name", "unknown") for prop in properties_using_struct
 ]
 raise HTTPException(
 status_code = 409,
 ...
 dependent_structs = []
 for other_struct in struct_type_registry.list_all():
 if other_struct.id != struct_type_id:
 for field in other_struct.fields:
 if field.data_type_id == struct_type_id:
 dependent_structs.append(other_struct.name)
 break
 ...
 # Log warning but allow deletion if dependency check fails
 logger.warning(
 f"Could not check dependencies for struct type {struct_type_id}: {e}"
 )
 # Continue with deletion despite dependency check failure
 ...
 await EventPublisher.publish_schema_event(
 "struct_type.deleted",
 {"struct_type_id": struct_type_id, "deleted_by": current_user},
 )
...
async def validate_struct_value(
 ...
 is_valid = is_valid, errors = errors, formatted_value = formatted_value
 )
...
async def check_nested_struct(
 ...
 if field.data_type_id.startswith("struct:") or struct_type_registry.exists(
 field.data_type_id
 ):
 has_nested = True
