...
class TestDeltaEncodingIntegration:
 ...
 return EnhancedDeltaEncoder(compression_threshold = 100, enable_binary_diff = True)

 @pytest.fixture
 def version_service(self):
 """Create version service with real dependencies"""
 cache = SmartCache(
 name = "test_delta", ttl = 60, use_redis = False # Local cache only for tests
 )
 return VersionService(repo_name = "test_repo", cache = cache)
 ...
 "old": {
 "users": [
 {"id": i, "name": f"User{i}", "data": "x" * 50}
 for i in range(20)
 ]
 },
 "new": {
 "users": [
 {"id": i, "name": f"User{i}", "data": "y" * 50}
 for i in range(20)
 ]
 },
 },
 ...
 delta_type, encoded, size = encoder.encode_delta(docs["old"], docs["new"])
 results[doc_type] = {
 "type": delta_type,
 "size": size,
 ...
 metrics.record_compression(doc_type, uncompressed_size, compressed_size)
 ...
 versions[0], versions[-1], [(i, d) for i, d in enumerate(deltas)]
 )
 ...
 "test_resource", doc.copy(), f"Update {i}", {"author": "test"}
 )
 ...
 {"key": "value"}, DeltaType.COMPRESSED_PATCH, b"corrupted data"
 )

 # Test unsupported delta type
 with pytest.raises(ValueError):
 encoder.decode_delta({"key": "value"}, "INVALID_TYPE", b"data")

 def test_performance_metrics(self, encoder, sample_documents):
 """Test performance metrics collection"""
 import time

 metrics = {"encoding_times": [], "decoding_times": [], "sizes": []}
 ...
 logger.info(
 f"Performance - Encoding: {avg_encoding:.3f}s, Decoding: {avg_decoding:.3f}s"
 )
...
class TestDeltaEncodingEdgeCases:
 ...
 old = {"level1": {"level2": {"level3": {"level4": {"value": "old"}}}}}
