...
async def benchmark_delta_encoding():
 ...
 # Real delta encoder for benchmarking
 class RealDeltaEncoder:
...
async def benchmark_time_travel():
 ...
 async def between(
 self, doc_id: str, start: datetime, end: datetime
 ) -> List[Any]:
 return [
 v
...
async def benchmark_vectors():
 ...
 k = 10
 similarities = [
 (i, cosine_similarity(query_vector, v)) for i, v in enumerate(vectors[:100])
 ]
 ...
 "Vector Similarity (Top-10 from 100)", test_batch_topk, iterations = 100
 )
...
async def benchmark_metadata():
 ...
 content.append(
 f"```@metadata:{frame_type} {frame_format}\n{frame_content}\n```\n"
 )
 ...
 await benchmark.measure(
 f"Metadata Parsing ({frame_count} frames)", test, iterations = 100
 )
...
async def main():
 ...
 if "avg_ms" in result:
 status = (
 "✅" if result["errors"] == 0 else f"⚠️ ({result['errors']} errors)"
 )
 print(
 f"{name:<40} {result['avg_ms']:<12.2f} {result['throughput_ops']:<15.1f} {status}"
 )
 ...
 if medium and large:
 print(
 f" • Delta encoding scales well: {small:.1f}ms → {medium:.1f}ms → {large:.1f}ms"
 )
