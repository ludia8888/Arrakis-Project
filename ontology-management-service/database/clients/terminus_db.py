...
class TerminusDBClient:
 ...
 logger.warning(
 "mTLS certificates not found, falling back to standard TLS"
 )
 ...
 logger.info(
 f"TerminusDB httpx.AsyncClient initialized - mTLS: {self.config.use_mtls}"
 )
 ...
 self._client = httpx.AsyncClient(
 base_url = self.config.endpoint, auth=(self.config.user, self.config.key)
 )
 ...
 raise ConnectionError("Client not initialized")
 add_span_attributes(
 {"db.name": db_name, "db.branch": branch_name, "db.operation": "query"}
 )
 ...
 raise ConnectionError("Client not initialized")
 add_span_attributes(
 {"db.name": db_name, "db.branch": branch_name, "document.id": document_id}
 )
 ...
 if not result:
 logger.warning(
 f"Document '{document_id}' not found or is empty in branch '{branch_name}'."
 )
 return None
 return result
 except httpx.HTTPStatusError as e:
 ...
 if e.response.status_code == 404:
 logger.warning(
 f"Document endpoint not found for branch '{branch_name}' or document '{document_id}' does not exist."
 )
 return None
 raise
 ...
 self, db_name: str, schema: Dict[str, Any], commit_msg: str = "Schema update"
 ):
 """스키마 업데이트"""
 if not self._client:
 raise ConnectionError("Client not initialized")
 ...
 payload = {"schema": schema, "commit_info": {"message": commit_msg}}
 ...
 response.raise_for_status()
 logger.info(
 f"Branch '{new_branch_name}' created from '{source_branch}' in db '{db_name}'."
 )
 return True
 except httpx.HTTPStatusError as e:
 if e.response.status_code == 400 and "already exists" in e.response.text:
 logger.warning(
 f"Branch '{new_branch_name}' already exists in db '{db_name}'."
 )
 return False
 logger.error(
 f"Failed to create branch '{new_branch_name}': {e.response.text}"
 )
 raise
 ...
 response.raise_for_status()
 logger.info(
 f"Branch '{branch_name}' deleted successfully from db '{db_name}'."
 )
 return True
 except httpx.HTTPStatusError as e:
 if e.response.status_code == 404:
 logger.warning(
 f"Branch '{branch_name}' not found for deletion in db '{db_name}'."
 )
 ...
 response.raise_for_status()
 logger.info(
 f"Document '{document_id}' deleted from branch '{branch_name}'."
 )
 return True
 except httpx.HTTPStatusError as e:
 if e.response.status_code == 404:
 logger.warning(
 f"Document '{document_id}' not found for deletion in branch '{branch_name}'."
 )
 return False
 logger.error(
 f"Failed to delete document '{document_id}': {e.response.text}"
 )
 raise
 ...
 payload = {
 "commit_info": {
 "message": commit_msg or f"Updated document {document.get('@id', '')}"
 },
 "document": document,
 }
 ...
 response.raise_for_status()
 logger.info(
 f"Document '{document.get('@id')}' updated in branch '{branch_name}'."
 )
 return response.json()
