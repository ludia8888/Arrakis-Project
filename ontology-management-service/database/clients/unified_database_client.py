...
class UnifiedDatabaseClient:
    """A unified client to interact with multiple database backends."""

    def __init__(self, terminus_client: Optional[TerminusDBClient] = None,
    ...
    async with _db_client_lock:
        ...
        if config.terminusdb:
            terminus_client_instance = TerminusDBClient(
                config=config.terminusdb,
                service_name=config.service.name
            )
        else:
            logger.warning("TerminusDB config not found, client will not be initialized.")
...
async def get_unified_database_client_legacy(config: AppConfig = None) -> "UnifiedDatabaseClient":
    """
