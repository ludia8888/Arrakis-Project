...
@router.get("/health")
async def health_check(svc: TerminusService = Depends(get_terminus_service)):
...
async def create_document(
    ...
    try:
        ...
        result = await svc.insert_document(
            db_name, payload, commit_msg=commit_msg, author=meta.author
        )
...
async def update_document(
    ...
    try:
        ...
        result = await svc.update_document(
            db_name, doc_id, updates, commit_msg=commit_msg, author=meta.author
        )
...
async def delete_document(
    ...
    try:
        ...
        result = await svc.delete_document(
            db_name, doc_id, commit_msg=commit_msg, author=meta.author
        )
...
async def update_schema(
    ...
    try:
        ...
        result = await svc.update_schema(
            db_name, schema, commit_msg=commit_msg, author=meta.author
        )
        return {"status": "updated", "result": result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
