...
class DocumentServicer(pb2_grpc.DocumentServiceServicer):
    ...
    async def Get(
        self, request: pb2.DocumentId, context: grpc.aio.ServicerContext
    ) -> pb2.Document:
        ...
        try:
            ...
            logger.debug(
                f"Get document: db={request.database}, id={request.id}, branch={branch}"
            )

            # Get document
            doc = await self.svc.get_document(
                request.database, request.id, branch=branch
            )
    ...
    async def Put(
        self, request: pb2.Document, context: grpc.aio.ServicerContext
    ) -> pb2.DocumentId:
        ...
        try:
            ...
            # Log context for debugging
            logger.debug(
                f"Put document: db={request.database}, author={author}, "
                f"branch={meta.branch if meta else 'default'}"
            )

            # Insert document
            result = await self.svc.insert_document(
                request.database, doc_data, commit_msg=commit_msg, author=author
            )

            # Extract document ID from result
            doc_id = doc_data.get("@id", "")

            return pb2.DocumentId(id=doc_id, database=request.database)
        except Exception as e:
    ...
    async def Delete(
        ...
        try:
            ...
            meta = request.meta
            commit_msg = (
                meta.commit_msg if meta else f"Delete document {request.id} via gRPC"
            )
            author = meta.author if meta else "system"

            # Delete document
            await self.svc.delete_document(
                request.database, request.id, commit_msg=commit_msg, author=author
            )
...
class QueryServicer(pb2_grpc.QueryServiceServicer):
    ...
    async def Execute(
        self, request: pb2.WOQL, context: grpc.aio.ServicerContext
    ) -> AsyncIterator[pb2.Document]:
        ...
        try:
            ...
            meta = request.meta
            commit_msg = (
                meta.commit_msg if meta and hasattr(meta, "commit_msg") else None
            )

            # Execute query
            result = await self.svc.query(
                request.database, query, commit_msg=commit_msg
            )
...
class SchemaServicer(pb2_grpc.SchemaServiceServicer):
    ...
    async def Get(
        self, request: pb2.SchemaRequest, context: grpc.aio.ServicerContext
    ) -> pb2.Schema:
        """Get database schema."""
        try:
            ...
            return pb2.Schema(
                json=json.dumps(schema).encode("utf-8"), database=request.database
            )
        except Exception as e:
    ...
    async def Update(
        self, request: pb2.Schema, context: grpc.aio.ServicerContext
    ) -> pb2.Empty:
        ...
        try:
            ...
            await self.svc.update_schema(
                request.database, schema, commit_msg=commit_msg, author=author
            )
...
class DataKernelServicer(pb2_grpc.DataKernelServiceServicer):
    ...
    async def CreateDocument(
        self,
        request: pb2.CreateDocumentRequest,
        context: grpc.aio.ServicerContext,
    ) -> pb2.CreateDocumentResponse:
        """Create a new document."""
        # Convert to simplified request
        doc = pb2.Document(
        ...
        try:
            ...
            if hasattr(self.svc, "terminus_client") and self.svc.terminus_client:
                ...
                branch_info = await self.svc.terminus_client.get_branch_info(
                    database=doc.database, branch=doc.meta.branch or "main"
                )
        ...
        return pb2.CreateDocumentResponse(document_id=doc_id.id, revision=revision)
    ...
    async def HealthCheck(
        ...
        try:
            ...
            return pb2.HealthCheckResponse(healthy=False, details={"error": str(e)})
...
async def serve(port: int = 50051):
    ...
    pb2_grpc.add_QueryServiceServicer_to_server(QueryServicer(terminus_svc), server)
    pb2_grpc.add_SchemaServiceServicer_to_server(SchemaServicer(terminus_svc), server)
