...
class ValidationConfig:
        ...
        return {
            "ObjectType": {
                ...
                "properties": {
                    ...
                    "display_name": {
                        "type": "string",
                        "maxLength": int(
                            os.getenv("SCHEMA_DISPLAY_NAME_MAX_LENGTH", "200")
                        ),
                    },
                    "description": {
                        "type": "string",
                        "maxLength": int(
                            os.getenv("SCHEMA_DESCRIPTION_MAX_LENGTH", "1000")
                        ),
                    },
            ...
            "ValidationRule": {
                ...
                "properties": {
                    ...
                    "rule_type": {"type": "string", "enum": self._get_rule_types()},
                    "condition": {"type": "object"},
                    "severity": {"type": "string", "enum": self._get_severity_levels()},
    ...
    def _load_validation_settings(self) -> Dict[str, Any]:
        ...
        return {
            "strict_mode": os.getenv("VALIDATION_STRICT_MODE", "false").lower()
            == "true",
            "enable_custom_rules": os.getenv(
                "ENABLE_CUSTOM_VALIDATION_RULES", "true"
            ).lower()
            == "true",
            "max_validation_errors": int(os.getenv("MAX_VALIDATION_ERRORS", "10")),
            "validation_timeout_seconds": float(
                os.getenv("VALIDATION_TIMEOUT_SECONDS", "30.0")
            ),
            "enable_schema_caching": os.getenv("ENABLE_SCHEMA_CACHING", "true").lower()
            == "true",
            "schema_cache_ttl_seconds": int(
                os.getenv("SCHEMA_CACHE_TTL_SECONDS", "300")
            ),
        }
    ...
    def _load_custom_config(self):
        ...
        try:
            ...
            logging.getLogger(__name__).warning(
                f"Failed to load custom validation config: {e}"
            )
