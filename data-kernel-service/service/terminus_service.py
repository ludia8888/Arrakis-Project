...
class TerminusService:
    ...
    async def get_document(
        ...
        try:
            ...
            query = {
                ...
                "document": {"@type": "woql:Node", "node": doc_id},
                "result": {"@type": "woql:Variable", "variable": "Document"},
            }
            ...
            if hasattr(self._client, "get_document"):
                try:
                    ...
                    logger.warning(
                        f"Native get_document failed: {native_error}, falling back to query"
                    )
            ...
            try:
                ...
                logger.warning(
                    f"Branch-aware query failed: {query_error}, trying simple query"
                )
    ...
    async def insert_document(
        ...
        query = {"@type": "InsertDocument", "document": document}
    ...
    async def update_document(
        ...
        query = {"@type": "UpdateDocument", "document": {"@id": doc_id, **updates}}
    ...
    async def delete_document(
        ...
        query = {"@type": "DeleteDocument", "document": doc_id}
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if self._health_cache is not None:
            ...
            if current_time - cache_time < self._health_cache_ttl:
                logger.debug(
                    f"Returning cached health check (age: {current_time - cache_time:.1f}s)"
                )
    ...
    async def _run_commit_hooks(
        self, db_name: str, diff: Dict[str, Any], commit_msg: str, author: str = None
    ):
        ...
        try:
            ...
            final_commit_msg = (
                commit_msg or CURRENT_COMMIT_MSG.get() or "No commit message"
            )
    ...
    async def _rollback_commit(self, db_name: str, commit_result: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(
                f"Performing rollback: database={db_name}, branch={branch}, "
                f"commit={commit_id}"
            )
            ...
            try:
                ...
                if hasattr(self._client, "reset"):
                    ...
                    if hasattr(self._client, "api_token") and self._client.api_token:
                        auth_header = {
                            "Authorization": f"Basic {self._client.api_token}"
                        }
                    ...
                    async with httpx.AsyncClient() as http_client:
                        ...
                        response = await http_client.post(
                            reset_url, headers=headers, json={}
                        )

                        if response.status_code == 200:
                            logger.info(
                                f"Successfully rolled back branch '{branch}' to before commit '{commit_id}'"
                            )
                        else:
                            logger.error(
                                f"Rollback API call failed: {response.status_code} - {response.text}"
                            )
    ...
    async def _audit_rollback_operation(
        self, db_name: str, branch: str, commit_id: str
    ):
        """Audit the rollback operation"""
        try:
            import os
            ...
            async with httpx.AsyncClient() as client:
                await client.post(
                    f"{audit_url}/audit/log", json=audit_event, timeout=5.0
                )
    ...
    async def _fallback_rollback(self, db_name: str, branch: str, commit_id: str):
        ...
        try:
            ...
            if hasattr(self._client, "get_commit_history"):
                ...
                commit_index = next(
                    (i for i, c in enumerate(history) if c.get("id") == commit_id), -1
                )
                if commit_index > 0:
                    ...
                    await self._restore_to_commit(
                        db_name, branch, previous_commit.get("id")
                    )

        except Exception as e:
            logger.error(f"Fallback rollback failed: {e}")

    async def _restore_to_commit(
        self, db_name: str, branch: str, target_commit_id: str
    ):
        ...
        try:
            ...
            for doc in documents.values():
                insert_query = {"@type": "InsertDocument", "document": doc}
            ...
            result = await self._client.query(
                db_name, restore_query, commit_msg=commit_msg
            )

            if result and result.get("status") != "error":
                logger.info(
                    f"Successfully restored {db_name}:{branch} to commit {target_commit_id}"
                )
                ...
                error_msg = (
                    result.get("message", "Unknown error") if result else "No result"
                )
...
def get_commit_context() -> Dict[str, str]:
    ...
    return {"author": CURRENT_AUTHOR.get(), "commit_msg": CURRENT_COMMIT_MSG.get()}
